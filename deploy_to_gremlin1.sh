...
EOF
  ...
  ssh "$GREMLIN1_HOST" "cd /etc/nixos && git add flake.nix && git commit -m 'Update flake with tinygrad backend' || true"

  log_success "NixOS configuration created with tinygrad backend"
}
