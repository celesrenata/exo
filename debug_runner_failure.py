...
def get_instance_details():
    ...
    try:
        ...
        if result.returncode != 0:
            print("Failed to fetch state: {result.stderr}")
            return None
        ...
        print("Type: {list(instance.keys())[0]}")

        instance_data = list(instance.values())[0]
        shard_assignments = instance_data.get("shardAssignments", {})

        print("Model: {shard_assignments.get('modelId', 'Unknown')}")
        print("Runners: {len(shard_assignments.get('runnerToShard', {}))}")
        ...
        for node_id, runner_id in node_to_runner.items():
            ...
            print("Node: {node_name} ({node_id[:8]}...)")
        ...
        print("Error getting instance details: {e}")
        return None
    ...
    try:
        ...
        for node_id, profile in profiles.items():
            ...
            if ram_total > 0:
                ...
                print(
                    f"  Memory: {ram_used / (1024**3):.1f}GB / {ram_total / (1024**3):.1f}GB ({ram_usage_pct:.1f}% used)"
                )
            ...
            print("  Engines: {', '.join(engines)} (selected: {selected_engine})")

    except Exception as e:
        print("Error checking system resources: {e}")
...
def check_recent_logs():
    ...
    for search_term in log_searches:
        try:
            ...
            print("Error searching logs for '{search_term}': {e}")
...
def suggest_fixes(details):
    ...
    failed_runners = [
        rid for rid, status in runners.items() if "RunnerFailed" in status
    ]

    if failed_runners:
        print("Found {len(failed_runners)} failed runner(s)")
        print("1. Delete and recreate the instance:")
        print(
            f"   curl -X DELETE http://10.1.1.12:52415/instance/{details['instance_id']}"
        )
        ...
        for node_id, runner_id in node_to_runner.items():
            if runner_id in failed_runners:
                node_name = get_node_name(
                    {"nodeProfiles": details.get("node_profiles", {})}, node_id
                )
                print("   - Check if {node_name} is accessible and healthy")
