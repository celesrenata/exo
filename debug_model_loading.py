...
def test_engine_selection():
    ...
    try:
        ...
        print("❌ Engine selection failed: {e}")
        traceback.print_exc()
        return None
...
def test_model_path_resolution():
    ...
    try:
        ...
        print("Model path for {test_model_id}: {model_path}")
        print("Path exists: {model_path.exists()}")

        return model_path

    except Exception as e:
        print("❌ Model path resolution failed: {e}")
        traceback.print_exc()
        return None
...
def test_bound_instance_creation():
    ...
    try:
        ...
        model_meta = ModelMeta(
            model_id="microsoft/DialoGPT-medium", storage_size=Memory.from_float_gb(1.0)
        )
        ...
        print("❌ Bound instance creation failed: {e}")
        traceback.print_exc()
        return None
...
def test_model_config_loading():
    ...
    try:
        ...
        print("Attempting to load config for {test_model_id}...")
        ...
        print("Vocab size: {getattr(config, 'vocab_size', 0)}")
        print("Hidden size: {getattr(config, 'hidden_size', 0)}")

        return config

    except Exception as e:
        print("❌ Model config loading failed: {e}")
        traceback.print_exc()
        return None
