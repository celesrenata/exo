...
def test_tokenizer_fix():
    ...
    try:
        ...
        shard_meta_0 = ShardMetadata(
            model_meta=model_meta, start_layer=0, end_layer=11, device_rank=0
        )

        bound_instance_0 = BoundInstance(
            shard_metadata=shard_meta_0, instance_id="test-0"
        )
        ...
        if tokenizer_0 is None:
            ...
            print("   Type: {type(tokenizer_0)}")

        # Test rank 1 (this was the problematic one)
        print("\n📍 Testing rank 1...")
        shard_meta_1 = ShardMetadata(
            model_meta=model_meta, start_layer=12, end_layer=23, device_rank=1
        )

        bound_instance_1 = BoundInstance(
            shard_metadata=shard_meta_1, instance_id="test-1"
        )
        ...
        print("   Rank 0: '{test_text}' -> {len(tokens_0)} tokens -> '{decoded_0}'")

        # Test rank 1 tokenizer
        tokens_1 = tokenizer_1.encode(test_text)
        decoded_1 = tokenizer_1.decode(tokens_1)
        print("   Rank 1: '{test_text}' -> {len(tokens_1)} tokens -> '{decoded_1}'")
        ...
        if tokens_0 == tokens_1 and decoded_0 == decoded_1:
            ...
            print("   Rank 0 tokens: {tokens_0}")
            print("   Rank 1 tokens: {tokens_1}")
