...
def test_torch_model_loading():
    ...
    try:
        ...
        print("Testing with model: {test_model_id}")
        ...
        print("❌ PyTorch model loading failed: {e}")
        traceback.print_exc()
        return False
...
def test_exo_torch_utils():
    ...
    try:
        ...
        print("❌ EXO torch utils test failed: {e}")
        traceback.print_exc()
        return False
...
def test_memory_constraints():
    ...
    try:
        ...
        print("Total RAM: {memory.total / (1024**3):.1f} GB")
        print("Available RAM: {memory.available / (1024**3):.1f} GB")
        print("Used RAM: {memory.percent}%")

        # Check disk space
        disk = psutil.disk_usage("/")
        print("Total disk: {disk.total / (1024**3):.1f} GB")
        print("Free disk: {disk.free / (1024**3):.1f} GB")

        # Check if we can allocate a reasonable tensor
        print("Testing tensor allocation...")
        test_tensor = torch.randn(1000, 1000)  # ~4MB tensor
        print("✅ Tensor allocation successful: {test_tensor.shape}")

        return True

    except Exception as e:
        print("❌ Memory constraint test failed: {e}")
        traceback.print_exc()
        return False
...
def main():
    ...
    print("\n=== Results ===")
    print("PyTorch model loading: {'✅ PASS' if torch_success else '❌ FAIL'}")
    print("EXO torch utils: {'✅ PASS' if exo_success else '❌ FAIL'}")
    print("Memory constraints: {'✅ PASS' if memory_success else '❌ FAIL'}")
