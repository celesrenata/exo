        ...
        if state_response.status_code == 200:
            ...
            for instance_id, instance_data in instances.items():
                if "CpuRingInstance" in instance_data:
                    ...
                    print("  Instance {instance_id[:8]}...:")
                    print("    Model: {model_id}")
                    print("    World size: {world_size}")
                    print(
                        f"    Nodes: {len(nodes)} ({', '.join([n[:8] + '...' for n in nodes])})"
                    )

                    if world_size > 1:
                        multinode_instances += 1
                        print("    ✅ Multi-node instance detected!")

                        # Check runner details
                        for runner_id, shard_info in shard_assignments[
                            "runnerToShard"
                        ].items():
                            if "PipelineShardMetadata" in shard_info:
                                rank = shard_info["PipelineShardMetadata"]["deviceRank"]
                                print("      Runner {runner_id[:8]}...: rank {rank}")
            ...
            print("❌ Failed to get system state: {state_response.status_code}")
            return False

    except Exception as e:
        print("❌ Exception checking system state: {e}")
        return False
    ...
    try:
        response = requests.post(
            ...
            json={
                ...
                "messages": [{"role": "user", "content": "Test multi-node stability"}],
                "max_tokens": 5,
                "stream": False,
            },
            timeout=30,
        ...
        if response.status_code == 200:
            ...
            print("✅ Inference successful: {content[:30]}...")
            return True
        else:
