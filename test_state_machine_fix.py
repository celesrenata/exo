...
def test_state_machine_logic():
    ...
    for i, case in enumerate(test_cases, 1):
        ...
        condition_met = (
            (current_status_connected and group_not_none) or 
        ...
        print(f"{i}. {case['name']}")
        print(f"   Status: {case['status']}, Group: {case['group']}")
        print(f"   Result: {result} (Expected: {expected}) {status_icon}")
...
def test_engine_initialization():
    ...
    try:
        ...
        if engine_type in ["torch", "cpu"] and group is None:
            ...
            print(f"✅ Engine returned group: {group}")
...
def main():
    ...
    print("📊 Test Summary")
    print("=" * 20)
    print(f"State Machine Logic: {'✅ PASS' if test1_passed else '❌ FAIL'}")
    print(f"Engine Initialization: {'✅ PASS' if test2_passed else '❌ FAIL'}")
