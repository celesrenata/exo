...
def check_runner_status():
    ...
    try:
        ...
        if result.returncode != 0:
            print("❌ Failed to get state: {result.stderr}")
            return {}, {}
        ...
        print("📊 Found {len(runners)} runners, {len(instances)} instances")
        ...
        for runner_id, runner_status in runners.items():
            ...
            if "RunnerFailed" in runner_status:
                ...
                print("   ✅ Status: {list(runner_status.keys())[0]}")

        # Check what model is being loaded
        for instance_id, instance_data in instances.items():
            print("\n📦 Instance {instance_id[:8]}...")
            if "CpuRingInstance" in instance_data:
                model_id = instance_data["CpuRingInstance"]["shardAssignments"][
                    "modelId"
                ]
                print("   📋 Model: {model_id}")

                shard_assignments = instance_data["CpuRingInstance"][
                    "shardAssignments"
                ]["runnerToShard"]
                for runner_id, shard_info in shard_assignments.items():
                    ...
                    print(
                        f"   🎯 Runner {runner_id[:8]}: rank {device_rank}, layers {start_layer}-{end_layer}"
                    )
...
def check_model_availability():
    ...
    if os.path.exists(model_path):
        ...
        print("   📄 Files: {len(files)} files found")
        ...
        if os.path.exists(parent_dir):
            print("   📁 Parent directory exists: {parent_dir}")
            models = [
                d
            ...
            print("   ❌ Parent directory doesn't exist: {parent_dir}")
    ...
    try:
        ...
        for line in meminfo.split("\n"):
            if "MemTotal:" in line:
                ...
                print("💾 Total Memory: {total_mem} MB ({total_mem / 1024:.1f} GB)")
            elif "MemAvailable:" in line:
                avail_mem = int(line.split()[1]) // 1024  # Convert to MB
                print(
                    f"💾 Available Memory: {avail_mem} MB ({avail_mem / 1024:.1f} GB)"
                )
        ...
        print(
            f"💽 Disk Space: {free // 1024 // 1024 // 1024} GB free of {total // 1024 // 1024 // 1024} GB total"
        )
    ...
    try:
        ...
        result = subprocess.run(
            [
                ...
                "5 minutes ago",
                "--grep",
                "bootstrap|Bootstrap|failed|error|ERROR|exception|Exception",
            ],
            capture_output=True,
            text=True,
            timeout=10,
        ...
        if result.stdout:
            ...
            for line in lines[-20:]:  # Show last 20 lines
                if any(
                    ...
                    for keyword in ["error", "failed", "exception", "bootstrap"]
                ):
                    print("   🔴 {line}")
        else:
...
def suggest_fixes():
    ...
    print(
        "2. 📥 Download the model manually: exo download meta-llama/Llama-3.1-8B-Instruct"
    )
    print(
        "3. 🧹 Clear model cache: rm -rf ~/.local/share/exo/models/meta-llama--Llama-3.1-8B-Instruct"
    )
    print("4. 🔍 Check logs in detail: journalctl -u exo -f")
    print(
        '5. 🎯 Try a smaller model: curl -X POST http://10.1.1.12:52415/v1/chat/completions -d \'{"model":"microsoft/DialoGPT-medium",...}\''
    )
...
def main():
    ...
    if failed_runners > 0:
        print("❌ {failed_runners} runners are failing during bootstrap")
        print("   Most likely cause: Model files not available or corrupted")
        print(
            "   Recommended action: Download the model manually or try a different model"
        )
    else:
