# Example NixOS configuration for PyTorch+IPEX backend with Intel Arc GPU
# This configuration enables the PyTorch+IPEX backend as the primary inference engine

{
  description = "NixOS configuration with PyTorch+IPEX backend for exo";
      ...
      modules = [
        ...
        # Main configuration
        {
          ...
          services.exo.intel = {
            ...
            # PyTorch+IPEX backend configuration (PRIMARY)
            pytorch_ipex = {
              # Enable PyTorch+IPEX backend
              enable = true;

              # Use PyTorch+IPEX as the preferred backend
              # This will try PyTorch+IPEX first, then fall back to tinygrad if unavailable
