...
def wait_for_service():
    ...
    for i in range(30):
        try:
            ...
            pass
        print(f"⏳ Waiting for EXO service... ({i+1}/30)")
        time.sleep(2)
    return False
...
def create_distributed_instance():
    ...
    if response.status_code == 200:
        ...
        print(f"✅ Instance creation initiated: {result['command_id']}")
        return True
    else:
    ...
    for instance_id, instance_data in instances.items():
        if 'CpuRingInstance' in instance_data:
            ...
            return {
                ...
                'world_size': list(shard_assignments['runnerToShard'].values())[0]['PipelineShardMetadata']['worldSize']
            }
...
def monitor_state_machine(max_wait_time=60):
    ...
    while time.time() - start_time < max_wait_time:
        ...
        print(f"   Runners: {instance_info['runners']}")
        print(f"   World Size: {instance_info['world_size']}")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'choices' in result and len(result['choices']) > 0:
                ...
                print(f"✅ Inference successful: {content}")
                return True
