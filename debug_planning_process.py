...
def test_planning_logic():
    ...
    try:
        ...
        print("❌ Planning logic test failed: {e}")
        traceback.print_exc()
        return False
...
def test_download_status_tracking():
    ...
    try:
        ...
        print("❌ Download status test failed: {e}")
        traceback.print_exc()
        return False
...
def test_model_path_resolution():
    ...
    try:
        ...
        print("EXO models directory: {EXO_MODELS_DIR}")

        test_model = "gpt2"
        model_path = build_model_path(test_model)
        print("Model path for {test_model}: {model_path}")
        ...
        if EXO_MODELS_DIR.exists():
            ...
            print("Downloaded models: {[m.name for m in models if m.is_dir()]}")
        else:
        ...
        print("❌ Model path test failed: {e}")
        traceback.print_exc()
        return False
...
def test_runner_status_flow():
    ...
    try:
        ...
        print("❌ Runner status test failed: {e}")
        traceback.print_exc()
        return False
...
def main():
    ...
    print("\n=== Results ===")
    print("Planning logic: {'✅ PASS' if planning_success else '❌ FAIL'}")
    print("Download status: {'✅ PASS' if download_success else '❌ FAIL'}")
    print("Model paths: {'✅ PASS' if path_success else '❌ FAIL'}")
    print("Runner status: {'✅ PASS' if status_success else '❌ FAIL'}")
    ...
    print(
        "   - Runner in correct state (Idle for single-node, Connected for multi-node)"
    )
