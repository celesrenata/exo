...
def check_process_status():
    ...
    try:
        ...
        if result.stdout:
            ...
            print(f"Found {len(pids)} EXO processes:")
            for pid in pids:
                # Get process info
...
def check_exo_config():
    ...
    for var_prefix in env_vars:
        matching_vars = {k: v for k, v in os.environ.items() if k.startswith(var_prefix)}
        if matching_vars:
...
def main():
    ...
    print(f"EXO Instance B6DAE586 Diagnostic Report")
