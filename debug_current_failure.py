...
def check_actual_model_being_used():
    ...
    try:
        ...
        print("Available model cards: {len(MODEL_CARDS)}")
        for model_id, card in list(MODEL_CARDS.items())[:5]:  # Show first 5
            print("  - {model_id}: {card.pretty_name}")
        ...
        print("❌ Error checking model configuration: {e}")
        traceback.print_exc()
        return None
...
def test_specific_model_loading(model_id):
    ...
    try:
        ...
        try:
            ...
            print("✅ Tokenizer loaded: vocab_size={tokenizer.vocab_size}")
        except Exception as e:
        ...
        try:
            ...
            print("✅ Basic inference successful: {output.logits.shape}")
...
def check_api_status():
    ...
    try:
        ...
        try:
            ...
            print("EXO API status: {response.status_code}")
            if response.status_code == 200:
                print("✅ EXO API is responding")
            else:
                print("⚠️ EXO API returned status {response.status_code}")
...
def check_planning_process():
    ...
    try:
        ...
        print("❌ Planning process check failed: {e}")
        traceback.print_exc()
        return False
...
def main():
    ...
    print("Model identification: {'✅ SUCCESS' if model_id else '❌ FAILED'}")
    print("Model loading: {'✅ SUCCESS' if model_success else '❌ FAILED'}")
    print("API status: {'✅ SUCCESS' if api_success else '❌ FAILED'}")
    print("Planning process: {'✅ SUCCESS' if planning_success else '❌ FAILED'}")

    if not model_success and model_id:
        print("\n🔍 LIKELY ROOT CAUSE:")
        print("The specific model '{model_id}' that EXO is trying to load is failing.")
        print("This is a persistent issue, not a transient dependency problem.")
        print("\nPossible causes:")
        print("1. Model files are corrupted or incomplete")
        print("2. Model is too large for available memory")
        print("3. Model format is incompatible with current transformers version")
        print("4. Model requires specific dependencies not available")
        print("5. Model path or permissions issue")

    elif not model_id:
        print("\n🔍 LIKELY ROOT CAUSE:")
        print("No model configuration found or model selection failing.")
        print("This could be a configuration or setup issue.")
