    ...
    try:
        ...
        for device_rank in [0, 1]:
            ...
            if tokenizer is None:
                print("    ❌ FAIL: Tokenizer is None on rank {device_rank}")
                return False
            else:
                print(
                    f"    ✅ PASS: Tokenizer loaded successfully on rank {device_rank}"
                )
                print("       Type: {type(tokenizer)}")
                print("       Vocab size: {len(tokenizer_raw.get_vocab())}")
                ...
                print(
                    f"       Test: '{test_text}' -> {len(tokens)} tokens -> '{decoded}'"
                )
