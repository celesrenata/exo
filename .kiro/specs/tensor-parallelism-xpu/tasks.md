...
- [ ] 1. Create NixOS Thunderbolt 4 networking module
  - [ ] 1.1 Create `nix/thunderbolt-net.nix` with module options and kernel module loading
  ...
  - [ ] 1.2 Implement systemd-networkd configuration for TB4 interfaces
  ...
  - [ ] 1.3 Implement firewall rules and systemd service for TB4 readiness
  ...
  - [ ]* 1.4 Write NixOS module evaluation tests
...
- [ ] 2. Implement TB4 topology discovery module
  - [ ] 2.1 Create `src/exo/worker/engines/pytorch_xpu/tb4_topology.py` with data models
  ...
  - [ ] 2.2 Implement `discover_tb4_topology()` async function
  ...
  - [ ] 2.3 Implement `select_tb4_interface()` function
  ...
  - [ ]* 2.4 Write property test for topology classification — Property 1
  ...
  - [ ]* 2.5 Write unit tests for TB4 topology discovery
...
- [ ] 4. Extend distributed.py with tensor-parallel process group management
  - [ ] 4.1 Add `TensorParallelGroupConfig` dataclass to `distributed.py`
  ...
  - [ ] 4.2 Implement `init_tensor_parallel_group()` function
  ...
  - [ ] 4.3 Implement `verify_tensor_parallel_group()` function
    - Each rank contributes its rank ID via all-reduce (sum)
    - Verify result equals world_size * (world_size - 1) / 2
    - Return True if verification passes, False otherwise
    - _Requirements: 9.3_

  - [ ]* 4.4 Write property test for rank derivation consistency — Property 6
  ...
  - [ ]* 4.5 Write unit tests for tensor-parallel process group
...
- [ ] 5. Implement TensorParallelShard with weight sharding
  - [ ] 5.1 Create `src/exo/worker/engines/pytorch_xpu/tensor_parallel_shard.py` with TPShardConfig
  ...
  - [ ] 5.2 Implement `TensorParallelShard.__init__()` and `shard_weights()`
  ...
  - [ ] 5.3 Implement `_column_parallel_linear()` and `_row_parallel_linear()` methods
  ...
  - [ ] 5.4 Implement `_all_reduce()` method with timeout handling
  ...
  - [ ] 5.5 Implement `forward()` method for tensor-parallel forward pass
  ...
  - [ ]* 5.6 Write property test for weight shard shapes — Property 2
  ...
  - [ ]* 5.7 Write property test for divisibility validation — Property 3
  ...
  - [ ]* 5.8 Write property test for forward pass equivalence — Property 4
  ...
  - [ ]* 5.9 Write unit tests for TensorParallelShard
...
- [ ] 7. Implement tensor-parallel generation pipeline
  - [ ] 7.1 Create `src/exo/worker/engines/pytorch_xpu/tensor_parallel_generator.py` with imports and constants
  ...
  - [ ] 7.2 Implement `tensor_parallel_generate()` function (rank 0 orchestrator)
  ...
  - [ ] 7.3 Implement `tensor_parallel_worker_loop()` function (non-rank-0 nodes)
  ...
  - [ ] 7.4 Implement error handling and graceful shutdown
  ...
  - [ ]* 7.5 Write unit tests for tensor-parallel generation
...
- [ ] 8. Implement placement and runner integration
  - [ ] 8.1 Define `TensorParallelInstance` dataclass in shared types
  ...
  - [ ] 8.2 Extend runner ConnectToGroup handler for tensor-parallel instances
  ...
  - [ ] 8.3 Extend placement module for tensor-parallel instance creation
  ...
  - [ ]* 8.4 Write property test for symmetric group validation — Property 5
  ...
  - [ ]* 8.5 Write unit tests for placement and runner integration
...
- [ ] 10. Implement TB4 link health monitoring and graceful degradation
  - [ ] 10.1 Add periodic TB4 link health checking to topology discoverer
  ...
  - [ ] 10.2 Implement graceful degradation on TB4 link failure
  ...
  - [ ]* 10.3 Write unit tests for link health monitoring
...
- [ ] 11. Update model cards and wire everything together
  - [ ] 11.1 Update Qwen3.5-4B model card for tensor parallelism
    - Set `supports_tensor = true` in the model card TOML
    - Verify 32 attention heads divisible by 4 (cluster size)
    - _Requirements: 7.3_

  - [ ] 11.2 Update Qwen3.5-2B model card for tensor parallelism
    - Set `supports_tensor = true` in the model card TOML
    - Verify 20 attention heads divisible by 4 (cluster size)
    - _Requirements: 7.4_

  - [ ] 11.3 Wire tensor-parallel generation into runner TextGeneration handler
...
- [ ] 12. Write integration tests for multi-process tensor parallelism
  - [ ]* 12.1 Write multi-process integration tests using torch.multiprocessing.spawn
  ...
  - [ ]* 12.2 Write integration test for hybrid parallelism coexistence
