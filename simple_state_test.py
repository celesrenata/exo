...
def test_state_machine_conditions():
    ...
    for i, case in enumerate(test_cases, 1):
        ...
        condition_met = (is_connected and group_not_none) or (is_idle and group_is_none)
        ...
        print("{i}. {case['name']}")
        print(
            f"   Condition: ({is_connected} AND {group_not_none}) OR ({is_idle} AND {group_is_none})"
        )
        print("   Result: {result} (Expected: {case['expected']}) {status_icon}")
