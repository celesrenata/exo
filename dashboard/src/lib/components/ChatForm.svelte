...
>
  ...
  >
    ...
    <div class="flex items-start gap-2 sm:gap-3 py-3 px-3 sm:px-4">
      ...
      <!-- Token budget inputs -->
      <input type="number" min="1" max="1048576" placeholder="Think tokens" aria-label="Thinking token budget" value={generationSettingsStore.thinking_token_budget ?? ""} disabled={!thinkingEnabled} onchange={(e) => { const v = (e.target as HTMLInputElement).value; generationSettingsStore.update({ thinking_token_budget: v === "" ? null : Math.min(parseInt(v), 1_048_576) }); }} class="w-20 bg-exo-medium-gray/50 border border-exo-medium-gray/50 rounded px-1.5 py-1 text-xs font-mono text-exo-yellow placeholder:text-exo-light-gray/40 focus:outline-none focus:border-exo-yellow/70 disabled:opacity-40 disabled:cursor-not-allowed" />
      <input type="number" min="1" max="1048576" placeholder="Max tokens" aria-label="Output token budget" value={generationSettingsStore.output_token_budget ?? ""} onchange={(e) => { const v = (e.target as HTMLInputElement).value; generationSettingsStore.update({ output_token_budget: v === "" ? null : Math.min(parseInt(v), 1_048_576) }); }} class="w-20 bg-exo-medium-gray/50 border border-exo-medium-gray/50 rounded px-1.5 py-1 text-xs font-mono text-exo-yellow placeholder:text-exo-light-gray/40 focus:outline-none focus:border-exo-yellow/70" />
