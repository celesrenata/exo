...
class DiagnosticLogger:
    ...
    def log_validation_complete(
        ...
        validation_overhead_ms = max(
            0.0, duration_ms - (token_count * 0.1)
        )  # Assume 0.1ms per token baseline
    ...
    def log_corruption_detected(
        ...
        self.log_event(
            ...
            details={
                ...
                "severity": severity,
                "token_positions": [
                    token.sequence_position for token in affected_tokens[:10]
                ],  # First 10
                "sample_corrupted_text": affected_tokens[0].text[:100]
                if affected_tokens
                else "",
            },
            device_id=device_id,
    ...
    def log_generation_pipeline_event(
        self,
        event_type: str,
        message: str,
    ...
    def log_synchronization_event(
        ...
        level = (
            ...
            if "timeout" in event_type or "failure" in event_type
    ...
    def get_diagnostic_summary(
        ...
        if validations:
            ...
            avg_duration = (
                sum(v.validation_duration_ms for v in validations) / total_validations
            )
            avg_overhead = (
                sum(v.validation_overhead_ms for v in validations) / total_validations
            )
        ...
        return {
            "summary_period": time_window.total_seconds()
            if time_window
            else "all_time",
...
def log_performance_event(metric_name: str, value: float, unit: str, **kwargs):
    ...
    get_diagnostic_logger().log_performance_metric(
        name=metric_name, value=value, unit=unit, **kwargs
    )
