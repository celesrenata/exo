...
from ..types.validation import (
    ValidationResult, CorruptionType, CorruptionSeverity, 
    EnhancedTokenChunk, ValidationStatus
)
...
class DistributedInferenceValidator:
    ...
    async def compare_single_vs_distributed(self, 
                                          prompt: str, 
                                          model_id: ModelId,
                                          seed: Optional[int] = None,
                                          max_tokens: int = 100) -> ComparisonResult:
        """
    ...
    async def run_deterministic_test(self, 
                                   ...
                                   model_id: ModelId,
                                   num_runs: int = 3,
                                   max_tokens: int = 100) -> DeterministicResult:
        """
    ...
    def validate_output_quality(self, 
                              ...
                              expected_output: Optional[str] = None) -> ValidationScore:
        """
        ...
        # Semantic coherence
        metric_scores[OutputQualityMetric.COHERENCE] = \
            self._check_semantic_coherence(output_text)
        
        # Token accuracy (if we have validation results)
        metric_scores[OutputQualityMetric.TOKEN_ACCURACY] = \
            self._check_token_accuracy(tokens)
        ...
        corruption_detected = any(
            token.validation_status == ValidationStatus.CORRUPTED 
            for token in tokens
        )
        encoding_issues = sum(
            1 for token in tokens 
            if token.validation_result and 
            token.validation_result.error_type == CorruptionType.ENCODING_CORRUPTION
        )
    ...
    def get_test_statistics(self) -> Dict[str, Any]:
        ...
        identical_outputs = sum(1 for result in self.test_history if result.are_identical)
        avg_similarity = sum(result.similarity_score for result in self.test_history) / total_tests
        
        # Quality metrics averages
        quality_averages = {}
        for metric in OutputQualityMetric:
            scores = [
                result.quality_metrics.get(metric, 0.0) 
                for result in self.test_history
            ]
            quality_averages[metric.value] = sum(scores) / len(scores) if scores else 0.0
        ...
        return {
            ...
            "identical_rate": identical_outputs / total_tests if total_tests > 0 else 0.0,
            ...
            "deterministic_success_rate": (
                ...
                if deterministic_tests > 0 else 0.0
            ),
            "last_test_time": (
    ...
    async def _generate_single_device(self, 
                                    prompt: str, 
                                    model_id: ModelId,
                                    max_tokens: int,
    ...
    async def _generate_distributed(self, 
                                  prompt: str, 
                                  model_id: ModelId,
                                  max_tokens: int,
                                  seed: Optional[int]) -> str:
    ...
    def _evaluate_quality_metrics(self, 
                                single_output: str, 
                                distributed_output: str) -> Dict[OutputQualityMetric, float]:
        ...
        metrics[OutputQualityMetric.SEMANTIC_SIMILARITY] = \
            self._calculate_similarity(single_output, distributed_output)
    ...
    def _check_sequence_completeness(self, tokens: List[EnhancedTokenChunk]) -> float:
        ...
        positions = [token.sequence_position for token in tokens if token.sequence_position > 0]
        if not positions:
    ...
    def _check_token_accuracy(self, tokens: List[EnhancedTokenChunk]) -> float:
        ...
        valid_tokens = sum(
            1 for token in tokens 
            if token.validation_status == ValidationStatus.VALID
        )
    ...
    def _count_sequence_gaps(self, tokens: List[EnhancedTokenChunk]) -> int:
        ...
        positions = [
            token.sequence_position for token in tokens 
            if token.sequence_position > 0
        ]
