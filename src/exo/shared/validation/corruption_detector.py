...
from ..types.validation import (
    CorruptionType, CorruptionSeverity, CorruptionReport, 
    EnhancedTokenChunk, ValidationResult, ValidationError
)
...
class CorruptionDetector:
    ...
    def __init__(self):
        ...
        self.corruption_patterns = [
            ...
            (re.compile(r'(.)\1{5,}'), CorruptionType.SEMANTIC_CORRUPTION, CorruptionSeverity.MEDIUM),
            
            # Random character sequences (high entropy)
            (re.compile(r'[^\w\s]{4,}'), CorruptionType.SEMANTIC_CORRUPTION, CorruptionSeverity.LOW),
            
            # Mixed scripts that shouldn't appear together
            (re.compile(r'[\u4e00-\u9fff][\u0400-\u04ff]'), CorruptionType.ENCODING_CORRUPTION, CorruptionSeverity.HIGH),
            
            # Control characters in text
            (re.compile(r'[\x00-\x08\x0b\x0c\x0e-\x1f\x7f-\x9f]'), CorruptionType.ENCODING_CORRUPTION, CorruptionSeverity.CRITICAL),
            
            # Malformed Unicode replacement characters
            (re.compile(r'\ufffd{2,}'), CorruptionType.ENCODING_CORRUPTION, CorruptionSeverity.HIGH),
            
            # Excessive whitespace or newlines
            (re.compile(r'\s{10,}'), CorruptionType.SEMANTIC_CORRUPTION, CorruptionSeverity.LOW),
            
            # Broken word boundaries (letters without spaces)
            (re.compile(r'[a-zA-Z]{50,}'), CorruptionType.SEMANTIC_CORRUPTION, CorruptionSeverity.MEDIUM),
        ]
    ...
    def detect_encoding_issues(self, text: str) -> CorruptionReport:
        ...
        if replacement_chars:
            issues.extend([(pos, "Unicode replacement character") for pos in replacement_chars])
        ...
        if issues:
            ...
            severity = CorruptionSeverity.HIGH if len(issues) > 5 else CorruptionSeverity.MEDIUM
    ...
    def detect_token_corruption(self, tokens: List[EnhancedTokenChunk]) -> List[CorruptionReport]:
        """
    ...
    def _detect_sequence_corruption(self, tokens: List[EnhancedTokenChunk]) -> CorruptionReport:
