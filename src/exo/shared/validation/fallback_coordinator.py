...
class FallbackCoordinator:
    ...
    def __init__(
        ...
        self.master_notification_callback: Optional[
            Callable[[FallbackStatus], None]
        ] = None
    ...
    def update_device_status(self, device_id: str, health: DeviceHealth):
        ...
        self.device_health_history[device_id] = [
            h
            for h in self.device_health_history[device_id]
    ...
    def report_corruption_event(self, corruption_report: CorruptionReport):
        ...
        logger.info(
            f"Corruption event reported: {corruption_report.corruption_type}, "
    ...
    def report_recovery_failure(self, recovery_result: RecoveryResult):
        ...
        if not recovery_result.success:
            ...
            logger.warning(
                f"Recovery failure reported: {recovery_result.action_taken}, "
    ...
    def should_trigger_fallback(self) -> FallbackDecision:
        ...
        if self.status != FallbackStatus.DISTRIBUTED:
            return FallbackDecision(
                False, reason="Already in fallback or transitioning"
            )
    ...
    async def initiate_fallback(
        self, trigger: Optional[FallbackTrigger] = None
    ) -> bool:
        """
        ...
        try:
            ...
            logger.info(
                f"Fallback completed successfully, primary device: {primary_device}"
            )
            return True
    ...
    async def attempt_recovery_to_distributed(self) -> bool:
        ...
        try:
            ...
            healthy_devices = [
                device_id
                for device_id, health_list in self.device_health_history.items()
    ...
    def _select_primary_device(self) -> Optional[str]:
        ...
        if device_scores:
            ...
            logger.info(
                f"Selected primary device: {best_device[0]} (score: {best_device[1]:.2f})"
            )
