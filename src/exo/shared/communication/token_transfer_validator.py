...
class TokenTransferValidator:
    ...
    def validate_token_transfer(
        self, 
        original: EnhancedTokenChunk, 
        received: EnhancedTokenChunk
    ) -> bool:
        """
        ...
        try:
            ...
            if (original.text != received.text or 
                original.token_id != received.token_id or
                original.idx != received.idx):
        ...
        try:
            ...
            if not result.is_valid:
                ...
                if result.checksum_mismatches:
                    details.append(f"Checksum mismatches: {len(result.checksum_mismatches)}")
    ...
    def _detect_token_corruption(self, token: EnhancedTokenChunk) -> bool:
        ...
        corruption_indicators = [
            ...
            sum(1 for c in text if not c.isalnum() and not c.isspace()) / max(len(text), 1) > 0.5,
            ...
            len(text.strip()) == 0 and hasattr(token, 'token_id') and token.token_id > 0
        ]
