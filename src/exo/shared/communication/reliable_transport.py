...
class ReliableMessageTransport:
    ...
    def __init__(
        ...
        self.transport_send_callback: Optional[
            Callable[[str, bytes], Awaitable[None]]
        ] = None
        self.transport_receive_callback: Optional[
            Callable[[NodeId], Awaitable[bytes]]
        ] = None
    ...
    async def send_with_validation(
        self, data: bytes, destination: NodeId, timeout: Optional[float] = None
    ) -> TransferResult:
        """
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if attempt < self.max_retries:
                    ...
                    backoff_time = min(0.1 * (self.backoff_multiplier**attempt), 2.0)
    ...
    async def _wait_for_acknowledgment(self, message_id: str, timeout: float) -> bool:
    ...
    async def _send_acknowledgment(self, message_id: str, destination: NodeId):
        ...
        try:
            ...
            logger.debug(
                f"Sent acknowledgment for message {message_id} to {destination}"
            )
        except Exception as e:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": (
                ...
                / (self.total_messages_sent + self.total_failures)
                if (self.total_messages_sent + self.total_failures) > 0
                else 0.0
            ),
        }
