...
class ReliableTopicRouter[T: CamelCaseModel](TopicRouter[T]):
    ...
    async def publish_bytes_reliable(self, data: bytes, source_node: Optional[NodeId] = None):
        """
        ...
        try:
            ...
            if self.enable_validation and source_node:
                ...
                if not validated_msg.verify_checksum():
                    logger.warning(f"Message integrity check failed for topic {self.topic.topic}")
    ...
    async def _send_out_reliable(self, item: T):
        ...
        while retry_count <= max_retries:
            try:
                logger.trace(f"Reliable TopicRouter {self.topic.topic} sending {item} (attempt {retry_count + 1})")
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            **self.message_stats,
            "transport_stats": self.reliable_transport.get_statistics() if self.reliable_transport else {}
        }
    ...
    async def _networking_recv_reliable(self):
        ...
        while True:
            try:
                ...
                logger.trace(f"Received message on {topic} with payload size {len(data)}")
    ...
    async def _networking_publish_reliable(self):
        ...
        with self.networking_receiver as networked_items:
            async for topic, data in networked_items:
                try:
                    logger.trace(f"Publishing message on {topic} with payload size {len(data)}")
                    ...
                    for attempt in range(max_retries + 1):
                        try:
                            ...
                            if attempt < max_retries:
                                ...
                                logger.error(f"Failed to publish on {topic} after {max_retries + 1} attempts: {e}")
                                self.router_stats["routing_errors"] += 1
...
def create_reliable_router(identity: Keypair, enable_reliability: bool = True) -> ReliableRouter:
    """
