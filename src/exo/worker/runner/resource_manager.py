...
class ResourceHandle:
    ...
    def __post_init__(self):
        ...
        if self.cleanup_func is None and self.async_cleanup_func is None:
            raise ValueError("Either cleanup_func or async_cleanup_func must be provided")
...
class ResourceManager:
    ...
    def __init__(self):
        ...
        self._resource_refs: Dict[str, Any] = {}  # Keep weak references to actual resources
    ...
    def register_resource(
        ...
        logger.debug(f"Registered resource {resource_id} (type: {resource_type}, order: {cleanup_order})")
        return handle
    ...
    async def cleanup_resources(self, timeout: float = 30.0) -> CleanupResult:
        ...
        async with self._lock:
            if self._cleanup_in_progress:
                ...
                return CleanupResult(success=False, errors=[RuntimeError("Cleanup already in progress")])
        ...
        try:
            ...
            logger.info(f"Starting cleanup of {len(cleanup_order)} resources with timeout {timeout}s")
            ...
            for resource_id in cleanup_order:
                ...
                if elapsed >= timeout:
                    logger.warning(f"Overall cleanup timeout exceeded, skipping remaining resources")
            ...
            logger.info(f"Cleanup completed in {result.total_time:.2f}s. "
    ...
    async def _cleanup_single_resource(self, handle: ResourceHandle, timeout: float) -> None:
        """
        ...
        try:
            if handle.async_cleanup_func:
                ...
                await asyncio.wait_for(
                    asyncio.to_thread(handle.cleanup_func),
                    timeout=timeout
                )
            else:
    ...
    async def cleanup_single_resource_by_id(self, resource_id: str, timeout: float = 10.0) -> bool:
        """
    ...
    async def wait_for_state(self, resource_id: str, target_state: ResourceState, timeout: float = 10.0) -> bool:
        """
