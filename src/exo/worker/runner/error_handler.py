...
class ErrorHandler:
    ...
    async def retry_with_backoff(
        ...
        **kwargs
        ...
        """
        retry_config = self._retry_configs.get(operation_name, self._default_retry_config)
        
        last_error = None
        
        for attempt in range(retry_config.max_attempts):
            try:
                logger.debug(f"Attempting {operation_name} (attempt {attempt + 1}/{retry_config.max_attempts})")
                ...
                if attempt < retry_config.max_attempts - 1:
                    ...
                    logger.warning(f"Operation {operation_name} failed (attempt {attempt + 1}): {e}. Retrying in {delay:.2f}s")
                    await asyncio.sleep(delay)
                else:
                    logger.error(f"Operation {operation_name} failed after {retry_config.max_attempts} attempts: {e}")
    ...
    def _determine_severity(self, error: Exception, recovery_action: RecoveryAction) -> ErrorSeverity:
        """Determine the severity of an error based on type and recovery action."""
        
        if recovery_action == RecoveryAction.SHUTDOWN:
            return ErrorSeverity.CRITICAL
        elif recovery_action in (RecoveryAction.ESCALATE, RecoveryAction.RESTART_COMPONENT):
    ...
    async def _handle_closed_resource_error(self, context: ErrorContext) -> RecoveryAction:
        ...
        if "shutdown" in context.operation.lower() or "cleanup" in context.operation.lower():
            return RecoveryAction.SKIP
        
        # During normal operation, this indicates a problem
        return RecoveryAction.FORCE_CLEANUP
    
    async def _handle_broken_resource_error(self, context: ErrorContext) -> RecoveryAction:
    ...
    async def _handle_timeout_error(self, context: ErrorContext) -> RecoveryAction:
        ...
        recent_timeouts = sum(
            ...
            if (
                isinstance(record.context.error, (asyncio.TimeoutError, TimeoutError)) and
                record.context.component == context.component and
                (datetime.now() - record.context.timestamp).total_seconds() < 300  # 5 minutes
            )
        )
    ...
    async def _handle_generic_error(self, context: ErrorContext) -> RecoveryAction:
        ...
        logger.debug(f"Handling generic error in {context.component}: {type(context.error).__name__}")
