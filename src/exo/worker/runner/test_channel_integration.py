...
from exo.worker.runner.queue_state_manager import QueueStateManager, get_queue_state_manager
from exo.worker.runner.enhanced_queue_operations import SafeQueueOperations, get_safe_queue_operations
...
async def test_basic_channel_operations():
    ...
    try:
        ...
        print(f"Channel info: {info.name}, state: {info.state}, messages sent: {info.messages_sent}")
...
async def test_queue_state_management():
    ...
    try:
        ...
        if metrics:
            print(f"Queue metrics: put={metrics.put_operations}, get={metrics.get_operations}, size={metrics.current_size}")
        
        # Test atomic closure
        success = await queue_manager.close_queue_atomically("test_queue", drain_timeout=2.0)
...
async def test_safe_queue_operations():
    ...
    try:
        ...
        print(f"Drain result: {result}, items drained: {len(drained_items)}, progress: {progress.items_drained}")
...
async def test_integration_scenario():
    ...
    try:
        ...
        channels = []
        for i in range(3):
            channel = await channel_manager.create_channel(f"integration_channel_{i}", buffer_size=10)
        ...
        # Close all queues
        for i in range(3):
            success = await queue_manager.close_queue_atomically(f"integration_queue_{i}")
...
async def main():
    ...
    if passed == total:
        print("🎉 All tests passed! The race-condition-free channel management system is working correctly.")
    else:
