...
from exo.worker.runner.resource_manager import ResourceManager, ResourceType, ResourceState, get_resource_manager
from exo.worker.runner.error_handler import ErrorHandler, RecoveryAction, get_error_handler
from exo.worker.runner.lifecycle_logger import LifecycleLogger, get_lifecycle_logger, LifecycleEventType
...
class RunnerSupervisor:
    ...
    _shutdown_coordinator: ShutdownCoordinator = field(default_factory=get_shutdown_coordinator, init=False)
    _resource_manager: ResourceManager = field(default_factory=get_resource_manager, init=False)
    _error_handler: ErrorHandler = field(default_factory=get_error_handler, init=False)
    _lifecycle_logger: LifecycleLogger = field(default_factory=get_lifecycle_logger, init=False)
    ...
    def create(
        ...
        *,
        ...
        self._shutdown_coordinator.register_shutdown_handler(self._handle_shutdown_signal)
        ...
        try:
            ...
            loop.create_task(self._lifecycle_logger.log_runner_created(
    ...
    def _configure_error_handling(self):
        ...
        self._error_handler.register_retry_config(
            "task_send",
            RetryConfig(max_attempts=2, base_delay=0.5, max_delay=5.0)
        )
        
        self._error_handler.register_retry_config(
            "event_forward",
            RetryConfig(max_attempts=2, base_delay=0.1, max_delay=2.0)
        )
        
        self._error_handler.register_retry_config(
            "process_join",
            RetryConfig(max_attempts=3, base_delay=2.0, max_delay=15.0, exponential_backoff=False)
        )
    ...
    async def _register_resources(self):
        ...
        try:
            ...
            await self._lifecycle_logger.log_resource_operation(
                ...
                resource_id=f"{self._runner_id}_process",
                operation="register",
                metadata={"resource_type": "process", "process_pid": self.runner_process.pid}
            )
            ...
            logger.error(f"Failed to register resources for runner {self._runner_id}: {e}")
    ...
    async def _cleanup_runner_process(self):
        ...
        try:
            ...
            logger.warning(f"Graceful join failed for runner process {self._runner_id}: {e}")
        
        # Escalate to terminate
        try:
            logger.warning(f"Runner process {self._runner_id} didn't shutdown gracefully, terminating")
            ...
            if not self.runner_process.is_alive():
                logger.debug(f"Runner process {self._runner_id} terminated successfully")
                return
                
        except Exception as e:
            logger.warning(f"Terminate failed for runner process {self._runner_id}: {e}")
        
        # Final escalation to kill
        try:
            logger.critical(f"Runner process {self._runner_id} didn't respond to SIGTERM, killing")
        ...
        logger.critical(f"Runner process {self._runner_id} didn't respond to SIGKILL. System resources may have leaked")
    ...
    async def _graceful_shutdown(self):
        ...
        try:
            ...
            if success:
                ...
                logger.warning(f"Graceful shutdown failed for runner {self._runner_id}, performing cleanup")
            ...
            cleanup_result = await self._resource_manager.cleanup_resources(timeout=30.0)
            ...
            if cleanup_result.success:
                ...
                logger.error(f"Resource cleanup failed for runner {self._runner_id}: {cleanup_result.errors}")
            ...
            shutdown_duration_ms = (time.perf_counter() - shutdown_start_time) * 1000
            logger.error(f"Error during graceful shutdown of runner {self._runner_id}: {e}")
            ...
            await self._lifecycle_logger.log_error_handling(
                ...
                component="RunnerSupervisor",
                operation="graceful_shutdown",
                recovery_action=recovery_action.value if hasattr(recovery_action, 'value') else str(recovery_action)
            )
            
            if recovery_action == RecoveryAction.FORCE_CLEANUP:
                logger.warning(f"Forcing cleanup for runner {self._runner_id}")
                try:
                    cleanup_result = await self._resource_manager.cleanup_resources(timeout=10.0)
                    if not cleanup_result.success:
                        logger.error(f"Force cleanup failed for runner {self._runner_id}: {cleanup_result.errors}")
                except Exception as cleanup_error:
                    logger.error(f"Force cleanup error for runner {self._runner_id}: {cleanup_error}")
    ...
    async def _handle_startup_failure(self, error: Exception, recovery_action: RecoveryAction):
    ...
    async def _health_monitor(self):
        ...
        while True:
            try:
                ...
                if self._shutdown_in_progress:
                    logger.debug(f"Stopping health monitoring due to shutdown for runner {self._runner_id}")
                    break
                
                # Perform health check
                health_check_start_time = time.perf_counter()
                health_status = await self._perform_health_check()
                health_check_duration_ms = (time.perf_counter() - health_check_start_time) * 1000
                ...
                await self._lifecycle_logger.log_health_check(
                    ...
                    health_score=health_status.get("metrics", {}).get("health_score", 0.0),
                ...
                if health_status["healthy"]:
                    ...
                    if consecutive_failures > 0:
                        logger.info(f"Runner {self._runner_id} health recovered after {consecutive_failures} failures")
                        consecutive_failures = 0
                    
                    logger.debug(f"Health check passed for runner {self._runner_id}")
                else:
                    consecutive_failures += 1
                    logger.warning(f"Health check failed for runner {self._runner_id} "
                    ...
                    if consecutive_failures < max_consecutive_failures:
                        recovery_success = await self._attempt_health_recovery(health_status)
                        if recovery_success:
                            logger.info(f"Health recovery successful for runner {self._runner_id}")
                            consecutive_failures = 0
                        else:
                            logger.warning(f"Health recovery failed for runner {self._runner_id}")
                    else:
                        logger.error(f"Runner {self._runner_id} failed {max_consecutive_failures} consecutive health checks, initiating shutdown")
                        await self._handle_persistent_failure()
                        break
                
                # Check for shutdown signals
                shutdown_phase = self._shutdown_coordinator.check_shutdown_signal(self._runner_id)
                if shutdown_phase:
                    logger.info(f"Detected shutdown signal for runner {self._runner_id}: {shutdown_phase}")
                    if self._tg:
                        self._tg.cancel_scope.cancel()
                    break
                
            except Exception as e:
                logger.error(f"Error in health monitor for runner {self._runner_id}: {e}")
                ...
                if recovery_action == RecoveryAction.ESCALATE:
                    logger.error(f"Health monitoring failed critically for runner {self._runner_id}")
                    break
    ...
    async def _perform_health_check(self) -> dict:
        ...
        try:
            ...
            if error_resources > 0:
                health_status["healthy"] = False
                health_status["issues"].append(f"{error_resources} resources in error state")
            ...
            if pending_count > 10:  # Configurable threshold
                health_status["healthy"] = False
                health_status["issues"].append(f"Too many pending tasks: {pending_count}")
            ...
            if isinstance(self.status, RunnerFailed):
                health_status["healthy"] = False
                health_status["issues"].append(f"Runner status is failed: {self.status}")
            ...
            if recent_errors > 5:  # Configurable threshold
                health_status["healthy"] = False
                health_status["issues"].append(f"Too many recent errors: {recent_errors}")
            ...
            health_status["metrics"]["shutdown_in_progress"] = self._shutdown_in_progress
            health_status["metrics"]["resources_registered"] = self._resources_registered
            ...
            if health_score < 0.7:  # 70% threshold
                ...
                if "Low health score" not in health_status["issues"]:
                    health_status["issues"].append(f"Low health score: {health_score:.2f}")
            
        except Exception as e:
            logger.error(f"Error performing health check for runner {self._runner_id}: {e}")
            health_status["healthy"] = False
        ...
        try:
            ...
            for issue in health_status["issues"]:
                if "Process not alive" in issue:
                    logger.warning(f"Cannot recover from dead process for runner {self._runner_id}")
                    recovery_success = False
                    
                elif "resources in error state" in issue:
                    logger.info(f"Attempting resource recovery for runner {self._runner_id}")
                    # Try to clean up error resources
                    try:
                        cleanup_result = await self._resource_manager.cleanup_resources(timeout=10.0)
                        if not cleanup_result.success:
                            logger.warning(f"Resource cleanup during recovery failed: {cleanup_result.errors}")
                            recovery_success = False
                    except Exception as e:
                    ...
                    logger.info(f"Clearing stale pending tasks for runner {self._runner_id}")
                    ...
                    logger.info(f"Clearing old error records for runner {self._runner_id}")
                    self._error_handler.clear_old_errors(hours=1)
            
            # Additional recovery actions
            if recovery_success:
                logger.info(f"Health recovery completed successfully for runner {self._runner_id}")
            else:
                logger.warning(f"Health recovery partially failed for runner {self._runner_id}")
            
        except Exception as e:
            logger.error(f"Error during health recovery for runner {self._runner_id}: {e}")
            recovery_success = False
    ...
    async def _handle_persistent_failure(self):
        ...
        try:
            ...
            await self._event_sender.send(
                RunnerStatusUpdated(
                    runner_id=self.bound_instance.bound_runner_id,
                    runner_status=RunnerFailed(error_message="Persistent health check failures"),
                )
            )
        except Exception as e:
    ...
    async def start_task(self, task: Task):
        ...
        # Check if shutdown is in progress
        if self._shutdown_in_progress:
            logger.warning(f"Cannot start task {task}, shutdown in progress for runner {self._runner_id}")
            return
        ...
        try:
            ...
            if recovery_action == RecoveryAction.SKIP:
                logger.info(f"Skipping task {task} due to closed resource (expected during shutdown)")
            else:
                logger.warning(f"Task {task} dropped, runner {self._runner_id} closed communication.")
        ...
        try:
            ...
            logger.error(f"Error waiting for task {task} completion on runner {self._runner_id}: {e}")
            self.pending.pop(task.task_id, None)
        ...
        with self._ev_recv as events:
            try:
                async for event in events:
                    # Check if shutdown is in progress
                    if self._shutdown_in_progress:
                        logger.debug(f"Stopping event forwarding due to shutdown for runner {self._runner_id}")
                        break
                    
                    if isinstance(event, RunnerStatusUpdated):
                        self.status = event.runner_status
                        logger.debug(f"Runner {self._runner_id} status updated: {event.runner_status}")
                        
                    if isinstance(event, TaskAcknowledged):
                        task_event = self.pending.pop(event.task_id, None)
                        if task_event:
                            task_event.set()
                        logger.debug(f"Task {event.task_id} acknowledged by runner {self._runner_id}")
                        continue
                    ...
                    try:
                        ...
                        if recovery_action == RecoveryAction.RETRY:
                            ...
                            try:
                                ...
                                logger.error(f"Event forwarding retry failed for runner {self._runner_id}: {retry_error}")
                        else:
                            logger.error(f"Error forwarding event from runner {self._runner_id}: {e}")
                        # Continue processing other events
                        
            except (ClosedResourceError, BrokenResourceError) as e:
                logger.info(f"Event channel closed for runner {self._runner_id}, checking runner status")
                ...
                logger.error(f"Unexpected error in event forwarding for runner {self._runner_id}: {e}")
    ...
    def __del__(self) -> None:
        ...
        if self.runner_process.is_alive():
            logger.warning(f"RunnerSupervisor {self._runner_id} was not stopped cleanly.")
    ...
    async def _check_runner(self, e: Exception) -> None:
        ...
        if self.runner_process.is_alive():
            logger.info(f"Runner {self._runner_id} was found to be alive, attempting to join process")
            try:
                await to_thread.run_sync(self.runner_process.join, 1)
            except Exception as join_error:
                logger.warning(f"Error joining runner process {self._runner_id}: {join_error}")
        ...
        try:
            ...
            logger.error(f"Failed to send failure status for runner {self._runner_id}: {send_error}")
    ...
    def get_runner_health(self) -> dict:
        ...
        try:
            ...
            basic_health = {
                ...
                "process_pid": self.runner_process.pid if self.runner_process.is_alive() else None,
            ...
            resource_states = self._resource_manager.get_resource_count_by_state()
            basic_health["resource_states"] = {state.value: count for state, count in resource_states.items()}
            ...
            shutdown_status = self._shutdown_coordinator.get_shutdown_status(self._runner_id)
            if shutdown_status:
    ...
    async def recover_runner(self) -> bool:
        ...
        if self.runner_process.is_alive():
            logger.info(f"Runner {self._runner_id} is still alive, performing health recovery")
            health_status = await self._perform_health_check()
            return await self._attempt_health_recovery(health_status)
        
        try:
            # Enhanced recovery for dead processes
            logger.warning(f"Runner process {self._runner_id} is dead, attempting full recovery")
            
            # Step 1: Clean up old resources
            logger.debug(f"Cleaning up old resources for runner {self._runner_id}")
            try:
                cleanup_result = await self._resource_manager.cleanup_resources(timeout=15.0)
                if not cleanup_result.success:
                    logger.warning(f"Resource cleanup during recovery had issues: {cleanup_result.errors}")
            except Exception as e:
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            self.runner_process.is_alive() and
            not self._shutdown_in_progress and
            self._resources_registered and
            not isinstance(self.status, RunnerFailed)
        )
