...
try:
    from exo.utils.channels import MpReceiver, MpSender, ClosedResourceError, WouldBlock
except ImportError:
...
from exo.worker.runner.queue_state_manager import QueueStateManager, QueueState, get_queue_state_manager
...
class OperationContext:
    ...
    started_at: datetime
    timeout: float
    process_id: int = field(default_factory=lambda: mp.current_process().pid if hasattr(mp.current_process(), 'pid') else 0)
...
class SafeQueueOperations:
    ...
    async def safe_put(
        ...
        try:
            ...
            if check_state:
                ...
                if queue_state and queue_state not in (QueueState.ACTIVE, QueueState.DRAINING):
        ...
        try:
            ...
            while time.time() - start_time < timeout:
                try:
                    ...
                    result, item, error = await self._perform_get_operation(queue, timeout=0.1)
            ...
            logger.info(f"Drained {progress.items_drained} items from queue {queue_id} in {total_time:.3f}s")
    ...
    async def cleanup_queue_with_dependencies(
        ...
        try:
            ...
            for dep in dependencies:
                if not await self._wait_for_dependency_ready(dep, timeout / len(dependencies) if dependencies else timeout):
                    logger.warning(f"Dependency {dep.dependency_queue} not ready for cleanup")
            ...
            try:
                if asyncio.iscoroutinefunction(cleanup_func):
                    ...
                    await asyncio.wait_for(
                        asyncio.to_thread(cleanup_func),
                        timeout=timeout
                    )
                
                cleanup_time = time.time() - start_time
                logger.info(f"Successfully cleaned up queue {queue_id} in {cleanup_time:.3f}s")
    ...
    async def add_queue_dependency(
        ...
        async with self._dependency_lock:
            ...
            logger.debug(f"Added dependency: {dependent_queue} -> {dependency_queue} ({dependency_type})")
        ...
        async with self._dependency_lock:
            if dependent_queue in self._dependencies:
                ...
                self._dependencies[dependent_queue] = {
                    dep for dep in self._dependencies[dependent_queue]
                ...
                logger.debug(f"Removed dependency: {dependent_queue} -> {dependency_queue}")
            ...
            if hasattr(queue, 'receive_nowait'):
                ...
                try:
                    ...
                    item = await asyncio.wait_for(
                        asyncio.to_thread(queue.get, True, timeout),
                        timeout=timeout
                    )
                    return OperationResult.SUCCESS, item, None
        ...
        try:
            ...
            if operation_type == OperationType.PUT:
                if result == OperationResult.SUCCESS:
                    ...
                    if metrics:
                        queue_state.update_metrics(put_operations=metrics.put_operations + 1)
                else:
                    metrics = queue_state.get_metrics()
                    if metrics:
                        queue_state.update_metrics(failed_operations=metrics.failed_operations + 1)
            elif operation_type == OperationType.GET:
                if result == OperationResult.SUCCESS:
                    metrics = queue_state.get_metrics()
                    if metrics:
                        queue_state.update_metrics(get_operations=metrics.get_operations + 1)
                else:
                    metrics = queue_state.get_metrics()
                    if metrics:
                        queue_state.update_metrics(failed_operations=metrics.failed_operations + 1)
    ...
    def get_operation_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_operation_time": sum(
                ...
                for op in operations if op.completed_at
