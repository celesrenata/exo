...
from .synchronization import CrossProcessEvent, CrossProcessLock, EventType
...
async def example_coordinated_shutdown():
    ...
    if success:
        ...
        if cleanup_result.success:
            print(
                f"Resource cleanup successful: {len(cleanup_result.cleaned_resources)} resources cleaned"
            )
        else:
            print(
    ...
    with shutdown_lock.acquire_context(timeout=10.0) as acquired:
        if acquired:
            ...
            shutdown_event.signal(
                ...
                {
                    "message": "Coordinated shutdown initiated",
