...
class StartupErrorHandler:
    ...
    async def handle_startup_with_retries(
        ...
        logger.info(
            f"Starting startup with error handling for runner {runner_id} "
        ...
        for attempt in range(self.max_retries + 1):  # +1 for initial attempt
            ...
            startup_attempt = StartupAttempt(
                attempt_number=attempt + 1, start_time=attempt_start_time
            )

            logger.info(
                f"Startup attempt {attempt + 1}/{self.max_retries + 1} for runner {runner_id}"
            )
            ...
            try:
                ...
                if not health_report.overall_health:
                    logger.warning(
                        f"Startup health check failed for runner {runner_id}: {health_report.issues}"
                    )
                    ...
                    if not resolved:
                        raise RuntimeError(
                            f"Failed to resolve startup health issues: {health_report.issues}"
                        )
                ...
                if success:
                    ...
                    logger.info(
                        f"Startup succeeded for runner {runner_id} on attempt {attempt + 1} "
                ...
                logger.warning(
                    f"Startup attempt {attempt + 1} failed for runner {runner_id}: {e}"
                )

                # Check if we should retry
                if attempt < self.max_retries:
                    should_retry, retry_delay = self._should_retry_startup(
                        startup_attempt, attempt
                    )

                    if should_retry:
                        logger.info(
                            f"Retrying startup for runner {runner_id} in {retry_delay:.1f}s"
                        )
                        await asyncio.sleep(retry_delay)
                        continue
                    else:
                        logger.error(
                            f"Startup failure not retryable for runner {runner_id}: {startup_attempt.failure_type}"
                        )
                        break
                else:
                    logger.error(
                        f"All startup attempts exhausted for runner {runner_id}"
                    )
                    break
    ...
    async def _perform_startup_health_check(
        ...
        overall_health = (
            ...
            and shutdown_coordinator_available
            and channel_manager_available
            and environment_configured
            and channels_registered
        )
        ...
        logger.debug(
            f"Startup health check for runner {runner_id}: "
    ...
    async def _resolve_health_issues(self, health_report: StartupHealthReport) -> bool:
        ...
        logger.info(
            f"Attempting to resolve {len(health_report.issues)} health issues for runner {runner_id}"
        )
        ...
        for issue in health_report.issues:
            try:
                if "Resource manager not available" in issue:
                    ...
                    logger.debug(
                        f"Resolved resource manager issue for runner {runner_id}"
                    )
                    ...
                    resolved_issues.append(issue)
                    logger.debug(
                        f"Resolved shutdown coordinator issue for runner {runner_id}"
                    )
                    ...
                    resolved_issues.append(issue)
                    logger.debug(
                        f"Resolved channel manager issue for runner {runner_id}"
                    )

                else:
                    logger.debug(
                        f"No automatic resolution available for issue: {issue}"
                    )

            except Exception as e:
                logger.warning(
                    f"Failed to resolve issue '{issue}' for runner {runner_id}: {e}"
                )
        ...
        if resolution_success:
            logger.info(
                f"Successfully resolved all health issues for runner {runner_id}"
            )
        else:
            logger.warning(
                f"Resolved {len(resolved_issues)}/{len(health_report.issues)} issues for runner {runner_id}"
            )
    ...
    def _should_retry_startup(
        self, attempt: StartupAttempt, attempt_number: int
    ) -> tuple[bool, float]:
        """
    ...
    async def _report_startup_failure(
        self, runner_id: str, event_sender: MpSender[Event]
    ) -> None:
        """
        ...
        if not attempts:
            ...
            total_duration = sum(
                attempt.duration for attempt in attempts if attempt.duration
            )
        ...
        logger.error(
            f"Comprehensive startup failure for runner {runner_id}: {error_message}"
        )
        ...
        try:
            ...
            logger.error(
                f"Failed to send startup failure event for runner {runner_id}: {e}"
            )
    ...
    def get_startup_statistics(self, runner_id: str) -> Dict[str, Any]:
        ...
        for attempt in failed_attempts:
            if attempt.failure_type:
                failure_types[attempt.failure_type.value] = (
                    failure_types.get(attempt.failure_type.value, 0) + 1
                )
        ...
        return {
            ...
            "average_attempt_duration": sum(a.duration for a in attempts if a.duration)
            / len(attempts)
            if attempts
            else 0,
...
async def bootstrap_runner_with_resource_management(
    ...
    runner_id = bound_instance.bound_runner_id
    logger.info(
        f"Starting bootstrap for runner {runner_id} with timeout {startup_timeout}s"
    )
    ...
    try:
        ...
        if elapsed_time >= startup_timeout:
            raise TimeoutError(
                f"Startup timeout exceeded during environment setup: {elapsed_time:.2f}s"
            )
        ...
        if elapsed_time >= startup_timeout:
            raise TimeoutError(
                f"Startup timeout exceeded before main logic: {elapsed_time:.2f}s"
            )

        logger.info(
            f"Bootstrap completed successfully for runner {runner_id} in {elapsed_time:.2f}s"
        )
            ...
            event_sender.send(
                RunnerStatusUpdated(
                    ...
                    runner_status=RunnerFailed(
                        error_message=f"Bootstrap failed: {str(e)}"
                    ),
                )
            )
        except Exception as send_error:
            logger.error(
                f"Failed to send failure event for runner {runner_id}: {send_error}"
            )
...
async def enhanced_cleanup_sequence(
    ...
    try:
        ...
        shutdown_success = await shutdown_coordinator.initiate_shutdown(
            runner_id,
            timeout=min(
                cleanup_timeout * 0.6, 10.0
            ),  # Use 60% of cleanup timeout for shutdown
        )

        if not shutdown_success:
            logger.warning(
                f"Shutdown coordination failed for runner {runner_id}, continuing with cleanup"
            )
        ...
        if remaining_time > 0:
            drain_timeout = min(
                remaining_time * 0.5, 5.0
            )  # Use up to 50% of remaining time for draining
        ...
        if remaining_time > 0:
            ...
            if cleanup_result.success:
                logger.info(
                    f"Resource cleanup completed successfully for runner {runner_id}"
                )
            else:
                logger.warning(
                    f"Resource cleanup had {len(cleanup_result.failed_resources)} failures for runner {runner_id}"
                )
                # Continue with manual cleanup for critical resources
                await _manual_cleanup_fallback(event_sender, task_receiver, runner_id)
        else:
            logger.warning(
                f"Cleanup timeout exceeded, performing emergency cleanup for runner {runner_id}"
            )
            await _manual_cleanup_fallback(event_sender, task_receiver, runner_id)

        total_cleanup_time = time.time() - cleanup_start_time
        logger.info(
            f"Enhanced cleanup completed for runner {runner_id} in {total_cleanup_time:.2f}s"
        )
        return True
...
async def _drain_receiver_safely(
    receiver: MpReceiver[Task], timeout: float, runner_id: str
) -> int:
    """
    ...
    try:
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if hasattr(receiver, "receive_nowait"):
                    ...
                    messages_drained += 1
                    logger.debug(
                        f"Drained task message for runner {runner_id}: {type(message)}"
                    )
                else:
        ...
        if messages_drained > 0:
            logger.debug(
                f"Drained {messages_drained} task messages for runner {runner_id}"
            )
...
async def _drain_sender_safely(
    sender: MpSender[Event], timeout: float, runner_id: str
) -> bool:
    """
            ...
            if hasattr(sender, "_queue") and hasattr(sender._queue, "qsize"):
                try:
                    ...
                    if queue_size == 0:
                        break
                    logger.debug(
                        f"Waiting for {queue_size} pending messages in sender for runner {runner_id}"
                    )
                except Exception:
                    break
            else:
...
async def _manual_cleanup_fallback(
    event_sender: MpSender[Event], task_receiver: MpReceiver[Task], runner_id: str
) -> None:
    """
    ...
    try:
        ...
        logger.debug(
            f"Manual cleanup: error closing task receiver for runner {runner_id}: {e}"
        )
    ...
    try:
        ...
        logger.debug(
            f"Manual cleanup: error closing event sender for runner {runner_id}: {e}"
        )
...
def entrypoint(
    ...
    try:
        ...
        success = asyncio.run(
            startup_handler.handle_startup_with_retries(
        ...
        try:
            ...
            if cleanup_success:
                logger.info(
                    f"Enhanced cleanup completed successfully for runner {runner_id}"
                )
            else:
            ...
            logger.error(
                f"Enhanced cleanup failed for runner {runner_id}: {cleanup_error}"
            )
            # Final emergency cleanup
            try:
                asyncio.run(
                    _manual_cleanup_fallback(event_sender, task_receiver, runner_id)
                )
            except Exception as emergency_error:
                logger.error(
                    f"Emergency cleanup failed for runner {runner_id}: {emergency_error}"
                )
...
def entrypoint_with_fallback_mechanisms(
    ...
    try:
        ...
        logger.warning(
            f"Primary entrypoint failed for runner {runner_id}: {primary_error}"
        )
        ...
        try:
            ...
            logger.error(
                f"Fallback startup also failed for runner {runner_id}: {fallback_error}"
            )
            ...
            try:
                ...
                logger.error(
                    f"Failed to send comprehensive failure event: {send_error}"
                )
