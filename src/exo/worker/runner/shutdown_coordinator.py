...
class ShutdownCoordinator:
    ...
    async def initiate_shutdown(self, runner_id: str, timeout: float = 30.0) -> bool:
        ...
        async with self._lock:
            ...
            signal_file = self._coordination_dir / f"shutdown_{runner_id}_{uuid4().hex[:8]}.signal"
            self._signal_files[runner_id] = signal_file
            
            logger.info(f"Initiating shutdown for runner {runner_id} with timeout {timeout}s")
    ...
    async def _execute_three_phase_shutdown(self, runner_id: str, timeout: float) -> bool:
    ...
    async def _phase_draining(self, runner_id: str) -> bool:
        ...
        drain_timeout = min(5.0, (state.timeout_at - datetime.now()).total_seconds() / 2)
        if drain_timeout > 0:
    ...
    def is_shutdown_in_progress(self, runner_id: str) -> bool:
        ...
        return state is not None and state.phase not in (ShutdownPhase.COMPLETE, ShutdownPhase.FAILED)
    ...
    async def cleanup_all(self) -> None:
        ...
        async with self._lock:
            ...
            try:
                if self._coordination_dir.exists() and not any(self._coordination_dir.iterdir()):
