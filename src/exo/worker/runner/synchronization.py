...
class CrossProcessLock:
    ...
    def __init__(self, lock_name: str, temp_dir: Optional[Path] = None, timeout: float = 30.0):
        """
    ...
    def acquire(self, timeout: Optional[float] = None) -> bool:
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                self._file_handle = os.open(self._lock_file, os.O_CREAT | os.O_WRONLY | os.O_TRUNC)
                ...
                lock_info = LockInfo(
                    ...
                    timeout_at=self._acquisition_time + timedelta(seconds=timeout) if timeout else None,
        ...
        try:
            if self._info_file.exists():
                ...
                if len(lines) >= 3:
                    ...
                    acquired_at = datetime.fromisoformat(lines[2].split(':', 1)[1].strip())
...
"""
    ...
    @contextmanager
    def acquire_context(self, timeout: Optional[float] = None) -> Generator[bool, None, None]:
...
class CrossProcessEvent:
    ...
    def signal(self, event_type: EventType, data: Optional[Dict[str, Any]] = None) -> bool:
        """
...
"""
            ...
            logger.debug(f"Signaled event {event_info.event_id} (type: {event_type.value})")
            return True
    ...
    def wait_for_event(self, event_type: EventType, timeout: float = 30.0) -> Optional[EventInfo]:
        """
        ...
        logger.debug(f"Timeout waiting for event {event_type.value} on {self.event_name}")
        return None
    
    def check_event(self, event_type: Optional[EventType] = None) -> Optional[EventInfo]:
        """
