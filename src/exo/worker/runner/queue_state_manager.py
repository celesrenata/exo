...
from exo.worker.runner.synchronization import (
    CrossProcessEvent,
    CrossProcessLock,
    EventType,
    SharedStateManager,
...
class SharedQueueState:
    ...
    def _initialize_state(self) -> None:
        ...
        with self._lock.acquire_context(timeout=5.0) as acquired:
            if not acquired:
                logger.warning(
                    f"Failed to acquire lock for initializing queue {self.queue_id}"
                )
                return
    ...
    def register_process(self, process_id: int) -> bool:
        ...
        with self._lock.acquire_context(timeout=5.0) as acquired:
            if not acquired:
                logger.warning(
                    f"Failed to acquire lock for registering process {process_id}"
                )
                return False

            try:
                self._state_manager.get_value("process_count", 0)
                active_processes = self._state_manager.get_value(
                ...
                if process_id not in active_processes:
                    ...
                    logger.debug(
                        f"Registered process {process_id} for queue {self.queue_id}"
                    )
    ...
    def unregister_process(self, process_id: int) -> bool:
        ...
        with self._lock.acquire_context(timeout=5.0) as acquired:
            if not acquired:
                logger.warning(
                    f"Failed to acquire lock for unregistering process {process_id}"
                )
                return False
            ...
            try:
                ...
                if process_id in active_processes:
                    ...
                    logger.debug(
                        f"Unregistered process {process_id} from queue {self.queue_id}"
                    )
    ...
    def update_state(
        self, new_state: QueueState, process_id: Optional[int] = None
    ) -> bool:
        """
        ...
        with self._lock.acquire_context(timeout=5.0) as acquired:
            if not acquired:
                logger.warning(
                    f"Failed to acquire lock for updating state to {new_state}"
                )
                return False
    ...
    def update_metrics(self, **kwargs) -> bool:
        ...
        with self._lock.acquire_context(timeout=5.0) as acquired:
            if not acquired:
                logger.warning("Failed to acquire lock for updating metrics")
                return False
    ...
    def _dict_to_metrics(self, metrics_dict: Dict[str, Any]) -> QueueMetrics:
        ...
        return QueueMetrics(
            ...
            health_status=QueueHealthStatus(
                metrics_dict.get("health_status", "healthy")
            ),
            created_at=datetime.fromisoformat(
                metrics_dict.get("created_at", datetime.now().isoformat())
            ),
            last_activity=datetime.fromisoformat(
                metrics_dict.get("last_activity", datetime.now().isoformat())
            ),
...
class QueueStateManager:
    ...
    async def register_queue(
        self, queue_id: str, max_size: int = 0
    ) -> SharedQueueState:
        """
    ...
    async def close_queue_atomically(
        self, queue_id: str, drain_timeout: float = 5.0
    ) -> bool:
        """
    ...
    async def recover_queue(self, queue_id: str) -> QueueRecoveryResult:
        ...
        if not queue_state:
            return QueueRecoveryResult(
                success=False, error=ValueError(f"Queue {queue_id} not found")
            )
        ...
        try:
            ...
            queue_state.update_metrics(
                error_count=0, last_error=None, failed_operations=0
            )
            ...
            if queue_state.update_state(QueueState.ACTIVE):
                ...
                logger.info(
                    f"Successfully recovered queue {queue_id} in {recovery_time:.3f}s"
                )

                return QueueRecoveryResult(success=True, recovery_time=recovery_time)
            else:
            ...
            return QueueRecoveryResult(
                success=False, recovery_time=recovery_time, error=e
            )
    ...
    async def _health_monitor_loop(self) -> None:
        ...
        while True:
            try:
                ...
                unhealthy_queues = [
                    queue_id
                    for queue_id, status in health_results.items()
    ...
    def get_summary_statistics(self) -> Dict[str, Any]:
        ...
        active_queues = sum(
            1 for m in all_metrics.values() if m and m.state == QueueState.ACTIVE
        )
        error_queues = sum(
            1 for m in all_metrics.values() if m and m.state == QueueState.ERROR
        )
