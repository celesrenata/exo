...
class ManagedChannel:
    ...
    def __init__(
        self, name: str, buffer_size: int = 1000, channel_id: Optional[str] = None
    ):
        """
    ...
    def get_sender(self) -> MpSender[Any]:
        ...
        with self._lock:
            if self._state != ChannelState.ACTIVE:
                raise RuntimeError(
                    f"Cannot get sender for channel {self.name} in state {self._state}"
                )
    ...
    def get_receiver(self) -> MpReceiver[Any]:
        ...
        with self._lock:
            if self._state != ChannelState.ACTIVE:
                raise RuntimeError(
                    f"Cannot get receiver for channel {self.name} in state {self._state}"
                )
    ...
    def send_safe(self, item: Any, timeout: float = 5.0) -> bool:
        ...
        with self._lock:
            if self._state not in (ChannelState.ACTIVE, ChannelState.DRAINING):
                logger.debug(
                    f"Cannot send to channel {self.name} in state {self._state}"
                )
                return False
    ...
    def drain(self, timeout: float = 5.0) -> DrainResult:
        ...
        with self._lock:
            if self._state == ChannelState.CLOSED:
                return DrainResult(
                    success=False, error=RuntimeError("Channel already closed")
                )
        ...
        try:
            ...
            while time.time() - start_time < timeout:
                try:
                    ...
                    messages_drained += 1
                    logger.debug(
                        f"Drained message from channel {self.name}: {type(item)}"
                    )
                except Exception:
            ...
            logger.debug(
                f"Drained {messages_drained} messages from channel {self.name} in {drain_time:.3f}s"
            )

            return DrainResult(
                success=True, messages_drained=messages_drained, drain_time=drain_time
            )
        ...
        try:
            ...
            if drain_first:
                ...
                if not drain_result.success:
                    logger.warning(
                        f"Failed to drain channel {self.name} before closing: {drain_result.error}"
                    )
...
class ChannelManager:
    ...
    async def create_channel(
        self, name: str, buffer_size: int = 1000
    ) -> ManagedChannel:
        """
    ...
    async def drain_channel(self, name: str, timeout: float = 5.0) -> List[Any]:
        ...
        if drain_result.success:
            logger.debug(
                f"Drained {drain_result.messages_drained} messages from channel {name}"
            )
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        for name, channel in self._channels.items():
            ...
            is_healthy = (
                ...
                and (datetime.now() - info.last_activity).total_seconds()
                < 300  # 5 minutes
            )
