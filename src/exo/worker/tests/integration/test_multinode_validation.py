...
class MultiNodeValidator:
    ...
    async def setup_multi_node_environment(self) -> bool:
        ...
        try:
            print(
                f"Setting up {self.config.node_count} nodes with {self.config.runners_per_node} runners each"
            )
            ...
            for node_id in range(self.config.node_count):
                ...
                for runner_id in range(self.config.runners_per_node):
                    ...
                    bound_instance = self._create_mock_bound_instance(
                        runner_name, self.config.parallelism_type
                    )
    ...
    def _create_mock_bound_instance(
        self, runner_id: str, parallelism_type: str
    ) -> BoundInstance:
        ...
        return BoundInstance(
            bound_runner_id=RunnerId(runner_id), bound_shard=Mock(), instance=instance
        )
    ...
    async def test_multi_node_instance_creation(self) -> bool:
        ...
        try:
            ...
            for node_name, runners in self.nodes.items():
                for runner_name, runner in runners.items():
                    ...
                    if not health.get("healthy", False):
                        print(
                            f"Runner {runner_name} on {node_name} is not healthy: {health}"
                        )
                        all_healthy = False
                    else:
            ...
            if all_healthy:
                ...
                print(
                    "✗ Multi-node instance creation test failed - some runners unhealthy"
                )
                return False
    ...
    async def test_inference_coordination(self) -> bool:
        ...
        try:
            ...
            if success_rate >= 0.8:  # 80% success rate threshold
                print(
                    f"✓ Inference coordination test passed ({success_count}/{total_tasks} tasks succeeded)"
                )
                return True
            else:
                print(
                    f"✗ Inference coordination test failed ({success_count}/{total_tasks} tasks succeeded)"
                )
                return False
    ...
    async def test_graceful_shutdown_scenarios(self) -> bool:
        ...
        if success_rate >= 0.75:  # 75% success rate threshold
            print(
                f"✓ Graceful shutdown test passed ({success_count}/{total_scenarios} scenarios)"
            )
            return True
        else:
            print(
                f"✗ Graceful shutdown test failed ({success_count}/{total_scenarios} scenarios)"
            )
            return False
    ...
    async def _test_normal_shutdown(self) -> bool:
        ...
        try:
            ...
            shutdown_tasks = []
            for _node_name, runners in self.nodes.items():
                for runner_name, _runner in runners.items():
    ...
    async def _test_partial_failure_shutdown(self) -> bool:
        ...
        try:
            ...
            for i, (_node_name, runners) in enumerate(self.nodes.items()):
                for j, (runner_name, _runner) in enumerate(runners.items()):
            ...
            for _node_name, runners in self.nodes.items():
                for runner_name, _runner in runners.items():
                    shutdown_tasks.append(
                        asyncio.create_task(
                            self.shutdown_coordinator.initiate_shutdown(
                                runner_id=runner_name, timeout=short_timeout
                            )
                        )
                    )
    ...
    async def test_error_handling_recovery(self) -> bool:
        ...
        try:
            ...
            if success_rate >= 0.5:  # 50% recovery rate threshold
                print(
                    f"✓ Error handling test passed ({success_count}/{total_scenarios} scenarios recovered)"
                )
                return True
            else:
                print(
                    f"✗ Error handling test failed ({success_count}/{total_scenarios} scenarios recovered)"
                )
                return False
    ...
    async def _simulate_resource_error(self):
        ...
        for _node_name, runners in self.nodes.items():
            for _runner_name, runner in runners.items():
    ...
    async def _simulate_communication_error(self):
        ...
        for i, (_node_name, sender) in enumerate(self.event_senders.items()):
    ...
    async def _check_system_recovery(self) -> bool:
        ...
        try:
            ...
            test_shutdown = await self.shutdown_coordinator.initiate_shutdown(
                runner_id="test_recovery", timeout=1.0
            )
            ...
            return (
                error_stats is not None
                and resource_states is not None
                and test_shutdown is not None
            )
    ...
    async def cleanup(self):
        ...
        try:
            ...
            # Close event senders
            for _node_name, sender in self.event_senders.items():
                try:
                    sender.close()
                except:
                    pass  # Ignore cleanup errors
...
async def test_pipeline_parallelism_multinode():
    ...
    config = MultiNodeTestConfig(
        node_count=2, runners_per_node=2, parallelism_type="pipeline"
    )
...
async def test_tensor_parallelism_multinode():
    ...
    config = MultiNodeTestConfig(
        node_count=2, runners_per_node=2, parallelism_type="tensor"
    )
...
async def test_stress_multinode():
    ...
    config = MultiNodeTestConfig(
        node_count=4, runners_per_node=3, test_timeout=60.0, shutdown_timeout=20.0
    )
    ...
    try:
        ...
        for cycle in range(3):
            ...
            assert creation_success, (
                f"Multi-node instance creation failed in cycle {cycle + 1}"
            )
