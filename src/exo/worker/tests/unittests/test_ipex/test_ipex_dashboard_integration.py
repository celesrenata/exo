...
class TestEngineInfoIntegration:
    ...
    def test_get_engine_info_with_ipex_available(self):
        ...
        with (
            ...
            patch(
                ...
                return_value=["ipex", "torch", "cpu"],
            ),
            patch(
                "exo.worker.engines.engine_utils.select_best_engine",
                return_value="ipex",
            ),
            patch(
    ...
    def test_get_engine_info_ipex_import_error(self):
        ...
        with (
            ...
            patch(
                "exo.worker.engines.ipex.utils_ipex.get_intel_gpu_info",
                side_effect=ImportError("IPEX not found"),
            ),
        ):
            engine_info = get_engine_info()
...
class TestIntelGPUInfoCollection:
    ...
    def test_get_intel_gpu_info_device_error(self):
        ...
        with (
            ...
            mock_xpu.get_device_properties.side_effect = RuntimeError(
                "Device properties failed"
            )

            # Mock memory usage
            mock_memory.return_value = {"allocated": 0, "reserved": 0, "free": 0}
...
class TestDashboardInstanceTypeSupport:
    ...
    def test_ipex_model_compatibility(self):
        ...
        assert (
            is_model_compatible("mlx-community/Llama-3.2-1B-Instruct-4bit", "ipex")
            is False
        )
    ...
    def test_compatible_models_filtering(self):
        ...
        with patch(
            "exo.worker.engines.engine_utils.select_best_engine", return_value="ipex"
        ):
...
class TestDashboardSystemInfoIntegration:
    ...
    def test_system_info_includes_intel_gpu(self):
        ...
        with patch(
            ...
            assert all(
                isinstance(device, dict) for device in gpu_info["intel_gpu_devices"]
            )
    ...
    def test_system_info_error_handling(self):
        ...
        with patch(
            "exo.worker.engines.ipex.utils_ipex.get_intel_gpu_info",
            side_effect=Exception("GPU info failed"),
        ):
...
class TestDashboardMonitoringIntegration:
    ...
    def test_gpu_health_monitoring_data_format(self):
        ...
        with patch(
            "exo.worker.engines.ipex.utils_ipex.monitor_intel_gpu_health"
        ) as mock_health:
            mock_health.return_value = {
                ...
                "memory_leak_info": {"leak_detected": False, "memory_growth": 0},
                "performance_alerts": [],
                "error_count": 0,
