...
from exo.worker.engines.engine_utils import detect_intel_gpu, detect_available_engines, select_best_engine
from exo.worker.engines.ipex.utils_ipex import check_ipex_availability, validate_intel_gpu_environment
...
class TestIntelGPUDetection:
    ...
    def test_detect_intel_gpu_no_ipex(self):
        ...
        with patch('intel_extension_for_pytorch', side_effect=ImportError("No module named 'intel_extension_for_pytorch'")):
            result = detect_intel_gpu()
    ...
    def test_detect_intel_gpu_tensor_operation_failure(self):
        ...
        with patch('intel_extension_for_pytorch') as mock_ipex, \
            ...
            with patch('torch.device'), \
                 patch('torch.tensor', side_effect=RuntimeError("Intel GPU operation failed")):
...
class TestEngineSelection:
    ...
    def test_select_best_engine_prefers_ipex_over_torch(self):
        ...
        with patch('exo.worker.engines.engine_utils.detect_available_engines') as mock_detect:
...
class TestIPEXAvailabilityCheck:
    ...
    def test_check_ipex_availability_tensor_failure(self):
        ...
        with patch('intel_extension_for_pytorch') as mock_ipex, \
            ...
            # Mock tensor operation failure
            with patch('torch.device'), \
                 patch('torch.tensor', side_effect=RuntimeError("Tensor operation failed")):
...
class TestIPEXEnvironmentValidation:
    ...
    def test_validate_intel_gpu_environment_corrupted_ipex(self):
        ...
        with patch('intel_extension_for_pytorch') as mock_ipex:
            ...
            assert "Intel Extension for PyTorch installation is corrupted" in str(exc_info.value)
    ...
    def test_validate_intel_gpu_environment_device_validation_failure(self):
        ...
        with patch('intel_extension_for_pytorch') as mock_ipex, \
            ...
            with patch('torch.device'), \
                 patch('torch.tensor', side_effect=RuntimeError("Device validation failed")):
