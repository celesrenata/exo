...
class TestIPEXWarmup:
    ...
    def test_warmup_inference_success(self, mock_model, mock_tokenizer, mock_sampler):
        ...
        with (
            ...
            tokens_generated = warmup_inference(
                mock_model, mock_tokenizer, mock_sampler
            )
    ...
    def test_warmup_inference_memory_error(
        self, mock_model, mock_tokenizer, mock_sampler
    ):
    ...
    def test_warmup_inference_general_error(
        self, mock_model, mock_tokenizer, mock_sampler
    ):
    ...
    def test_warmup_inference_early_termination(
        self, mock_model, mock_tokenizer, mock_sampler
    ):
...
class TestIPEXGeneration:
    ...
    def test_ipex_generate_single_device(
        self, mock_model, mock_tokenizer, mock_sampler, mock_task
    ):
        """Test IPEX generation on single device."""
        with patch(
            "exo.worker.engines.ipex.generator.generate._ipex_generate_single_device"
        ) as mock_single:
            ...
            # Test generation
            responses = list(
                ipex_generate(mock_model, mock_tokenizer, mock_sampler, mock_task)
            )
    ...
    def test_ipex_generate_distributed(
        self, mock_model, mock_tokenizer, mock_sampler, mock_task
    ):
        ...
        with patch(
            "exo.worker.engines.ipex.generator.generate._ipex_generate_distributed"
        ) as mock_distributed:
            # Mock generator response
            def mock_generator():
                yield GenerationResponse(
                    text="Distributed", token=20, finish_reason=None
                )
                yield GenerationResponse(
                    text=" response", token=21, finish_reason="length"
                )
    ...
    def test_ipex_generate_memory_error(
        self, mock_model, mock_tokenizer, mock_sampler, mock_task
    ):
    ...
    def test_ipex_generate_inference_error(
        self, mock_model, mock_tokenizer, mock_sampler, mock_task
    ):
...
class TestIPEXSingleDeviceGeneration:
    ...
    def test_single_device_generation_success(
        self, mock_model, mock_tokenizer, mock_sampler, mock_task
    ):
        ...
        with (
            ...
            patch("torch.xpu.empty_cache"),
            patch(
                "exo.worker.engines.ipex.generator.generate.monitor_intel_gpu_health"
            ) as mock_health,
            patch(
                "exo.worker.engines.ipex.generator.generate.handle_intel_gpu_health_issues"
            ),
        ):
            # Mock tensor operations
            mock_input_ids = Mock()
    ...
    def test_single_device_generation_eos_termination(
        self, mock_model, mock_tokenizer, mock_sampler, mock_task
    ):
    ...
    def test_single_device_generation_health_issues(
        self, mock_model, mock_tokenizer, mock_sampler, mock_task
    ):
...
class TestIPEXDistributedGeneration:
    ...
    def test_distributed_generation_pipeline_rank_0(
        self, mock_distributed_model, mock_tokenizer, mock_sampler, mock_task
    ):
        ...
        with (
            ...
            patch("torch.xpu.empty_cache"),
            patch(
                "exo.worker.engines.ipex.generator.generate.intel_gpu_distributed_barrier"
            ),
        ):
    ...
    def test_distributed_generation_tensor_parallelism(
        self, mock_distributed_model, mock_tokenizer, mock_sampler, mock_task
    ):
    ...
    def test_distributed_generation_memory_error(
        self, mock_distributed_model, mock_tokenizer, mock_sampler, mock_task
    ):
        ...
        with (
            ...
            # Mock memory error during forward pass
            mock_distributed_model.side_effect = torch.xpu.OutOfMemoryError(
                "Distributed GPU out of memory"
            )
            ...
            assert "Intel GPU out of memory during distributed generation" in str(
                exc_info.value
            )

    def test_distributed_generation_fallback_to_single(
        self, mock_distributed_model, mock_tokenizer, mock_sampler, mock_task
    ):
        ...
        with (
            ...
            def mock_single_generator():
                yield GenerationResponse(
                    text="fallback", token=99, finish_reason="stop"
                )
...
class TestIPEXSimpleGeneration:
    ...
    def test_ipex_generate_simple_memory_error(self, mock_model, mock_tokenizer):
        ...
        with (
            ...
            mock_model.generate.side_effect = torch.xpu.OutOfMemoryError(
                "Simple generation out of memory"
            )

            with pytest.raises(IPEXMemoryError) as exc_info:
                ipex_generate_simple(mock_model, mock_tokenizer, "Test prompt")

            assert "Intel GPU out of memory during simple generation" in str(
                exc_info.value
            )
