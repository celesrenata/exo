...
from exo.worker.tests.constants import MODEL_A_ID, NODE_A, RUNNER_1_ID
...
class TestIPEXModelLoading:
    ...
    def test_initialize_ipex_success(self, mock_bound_instance):
        ...
        with (
            patch("exo.worker.engines.ipex.utils_ipex.setup_ipex_logging"),
            patch("exo.worker.engines.ipex.utils_ipex.validate_intel_gpu_environment"),
            patch(
                "exo.worker.engines.ipex.utils_ipex.enable_intel_gpu_kernel_optimizations"
            ),
            patch(
                "exo.worker.engines.ipex.utils_ipex.create_intel_gpu_optimized_sampler"
            ) as mock_sampler,
            patch(
                "exo.worker.engines.ipex.utils_ipex.initialize_single_device_ipex"
            ) as mock_init,
        ):
            # Mock return values
    ...
    def test_initialize_ipex_distributed(self, mock_bound_instance):
        ...
        with (
            ...
            patch(
                "exo.worker.engines.ipex.utils_ipex.initialize_distributed_ipex"
            ) as mock_init_dist,
        ):
            # Mock return values
            mock_model = Mock(spec=IPEXModel)
            ...
            mock_init_dist.assert_called_once_with(
                mock_bound_instance, mock_sampler_func
            )
    ...
    def test_initialize_ipex_validation_failure(self, mock_bound_instance):
        ...
        with (
            patch("exo.worker.engines.ipex.utils_ipex.setup_ipex_logging"),
            patch(
                "exo.worker.engines.ipex.utils_ipex.validate_intel_gpu_environment",
    ...
    def test_load_ipex_model_success(self, mock_model_path):
        ...
        with (
            ...
            patch(
                "exo.worker.engines.ipex.utils_ipex.detect_model_quantization"
            ) as mock_detect_quant,
            patch(
                "exo.worker.engines.ipex.utils_ipex.get_optimal_dtype_for_intel_gpu"
            ) as mock_dtype,
            patch(
                "exo.worker.engines.ipex.utils_ipex.optimize_model_for_intel_gpu"
            ) as mock_optimize,
...
class TestIPEXTokenizerWrapper:
    ...
    def test_tokenizer_wrapper_encode(self, mock_tokenizer):
        ...
        mock_tokenizer.encode.assert_called_once_with(
            "test text", add_special_tokens=True
        )
    ...
    def test_tokenizer_wrapper_apply_chat_template(self, mock_tokenizer):
        ...
        result = wrapper.apply_chat_template(
