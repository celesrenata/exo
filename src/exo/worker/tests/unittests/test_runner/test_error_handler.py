...
class TestErrorHandler:
    ...
    async def test_retry_with_backoff_success(self, handler):
        ...
        result = await handler.retry_with_backoff(
            operation_that_succeeds_on_second_try,
            "test_operation"
        )
    ...
    def test_determine_severity(self, handler):
        ...
        assert handler._determine_severity(Exception(), RecoveryAction.SHUTDOWN) == ErrorSeverity.CRITICAL
        assert handler._determine_severity(Exception(), RecoveryAction.ESCALATE) == ErrorSeverity.HIGH
        assert handler._determine_severity(Exception(), RecoveryAction.RESTART_COMPONENT) == ErrorSeverity.HIGH
        assert handler._determine_severity(Exception(), RecoveryAction.FORCE_CLEANUP) == ErrorSeverity.MEDIUM
        assert handler._determine_severity(Exception(), RecoveryAction.RETRY) == ErrorSeverity.LOW
        assert handler._determine_severity(Exception(), RecoveryAction.SKIP) == ErrorSeverity.LOW
