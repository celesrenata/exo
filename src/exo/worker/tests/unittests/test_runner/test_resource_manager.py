...
class TestResourceManager:
    ...
    def test_register_resource_validation_error(self, manager):
        ...
        with pytest.raises(
            ValueError,
            match="Either cleanup_func or async_cleanup_func must be provided",
        ):
            manager.register_resource(
                resource=resource, resource_type=ResourceType.CUSTOM, cleanup_order=10
            )
    ...
    async def test_cleanup_single_resource_timeout(self, manager):
        ...
        success = await manager.cleanup_single_resource_by_id(
            "test_resource_1", timeout=0.1
        )
    ...
    async def test_wait_for_state_timeout(self, manager):
        ...
        result = await manager.wait_for_state(
            "test_resource", ResourceState.CLOSED, timeout=0.1
        )
