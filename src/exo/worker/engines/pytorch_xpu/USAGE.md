...
The PyTorchXPUBackend provides Intel Arc GPU support for exo using PyTorch and Intel Extension for PyTorch (IPEX).
...
```python
from exo.worker.engines.pytorch_xpu import PyTorchXPUBackend

# Create backend instance
backend = PyTorchXPUBackend()
...
```python
from exo.worker.engines.pytorch_xpu import DeviceManager, DeviceType
...
```python
from exo.worker.engines.pytorch_xpu import (
    DeviceError,
    ModelError,
    InferenceError,
    CacheError,
...
async def generate_text(prompt: str, max_tokens: int = 50) -> str:
    """Generate text using PyTorchXPUBackend."""
...
```
RuntimeError: PyTorch is required for PyTorchXPUBackend
```
