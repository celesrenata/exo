...
from exo.worker.engines.ipex.utils_ipex import MAX_TOKENS, apply_chat_template, handle_ipex_fallback
...
def warmup_inference(
    ...
    from exo.worker.engines.ipex.utils_ipex import log_ipex_performance_metrics, log_ipex_error_context
    ...
    try:
        ...
        input_ids = torch.tensor([tokenizer.encode(warmup_prompt)], dtype=torch.long, device=device)
        ...
        log_ipex_error_context(e, "warmup", {
        ...
        raise IPEXMemoryError(
            f"Intel GPU out of memory during warmup: {e}",
            device_id=0
        )
    ...
    logger.info(f"Generated {tokens_generated} warmup tokens with IPEX in {warmup_duration:.2f}s")
...
def ipex_generate(
    ...
    model_info = {
        ...
        "distributed": hasattr(model, '_ipex_dist_group') and model._ipex_dist_group is not None
    }
    ...
    try:
        ...
        is_distributed = hasattr(model, '_ipex_dist_group') and model._ipex_dist_group is not None
        
        if is_distributed:
            logger.info("Running distributed IPEX inference")
            for response in _ipex_generate_distributed(model, tokenizer, sampler, task, prompt, device):
                tokens_generated += 1
                yield response
            ...
            for response in _ipex_generate_single_device(model, tokenizer, sampler, task, prompt, device):
                tokens_generated += 1
                yield response
                if response.finish_reason is not None:
                    break
        ...
        log_ipex_error_context(e, "inference_generation", {
            "tokens_generated": tokens_generated,
        ...
        with torch.xpu.amp.autocast(enabled=True, dtype=torch.float16):
            while generated_tokens < max_tokens:
                ...
                if generated_tokens > 0 and generated_tokens % 50 == 0:
                    health_summary = monitor_intel_gpu_health(0)
                    if health_summary.get("overall_health") != "healthy":
                        logger.warning(f"Intel GPU health issues detected during generation at token {generated_tokens}")
                        if not handle_intel_gpu_health_issues(health_summary):
                            logger.error("Could not resolve Intel GPU health issues, continuing generation")
                ...
                try:
                    ...
                    logger.error(f"Intel GPU out of memory during generation at token {generated_tokens}: {e}")
                    # Try to recover
                    torch.xpu.empty_cache()
                    health_summary = monitor_intel_gpu_health(0)
                    if handle_intel_gpu_health_issues(health_summary):
                        logger.info("Attempting to continue generation after memory recovery")
                        continue
                    else:
                        raise IPEXMemoryError(
                            f"Intel GPU out of memory during generation: {e}",
                    ...
                    logger.error(f"Error during Intel GPU inference at token {generated_tokens}: {e}")
                    ...
                    if health_summary.get("overall_health") == "healthy":
                        ...
                        if handle_intel_gpu_health_issues(health_summary):
                            logger.info("Recovered from Intel GPU health issues, continuing")
                            continue
                        else:
                            raise IPEXInferenceError(
...
def _ipex_generate_distributed(
    ...
    try:
        ...
        logger.info(f"Distributed IPEX generation: rank={rank}, world_size={world_size}, type={parallelism_type}")
        ...
        logger.info(f"Starting distributed IPEX generation with max_tokens={max_tokens}")
                ...
                while generated_tokens < max_tokens:
                    ...
                    if parallelism_type == "pipeline":
                        ...
                        if rank == 0:
                            ...
                            hidden_states = torch.empty((1, input_ids.size(1) if input_ids is not None else 1, model.config.hidden_size), 
                            ...
                            if rank < world_size - 1:
                                dist.send(outputs.last_hidden_state, dst=rank+1, group=dist_group)
                    ...
                    if (parallelism_type == "pipeline" and rank == world_size - 1) or parallelism_type == "tensor":
                        # Apply Intel GPU optimized sampling
                        next_token = sampler(logits)
                        next_token_id = next_token.item()
                        ...
                        dist.broadcast(token_tensor, src=world_size-1 if parallelism_type == "pipeline" else 0, group=dist_group)
                    ...
                    if rank == 0 or (parallelism_type == "pipeline" and rank == world_size - 1):
                        try:
                            # Decode the new token
                            token_text = tokenizer.decode([next_token_id], skip_special_tokens=True)
                            ...
                            if finish_reason is not None:
                                logger.info(f"Distributed IPEX generation finished: {finish_reason}")
                                break
        ...
        logger.info(f"Distributed IPEX generation complete. Generated {generated_tokens} tokens.")
        ...
        try:
            ...
            pass
        raise IPEXMemoryError(
            f"Intel GPU out of memory during distributed generation: {e}",
            device_id=0
        )
    except Exception as e:
        ...
        if hasattr(model, '_ipex_rank') and model._ipex_rank == 0:
            ...
            try:
                yield from _ipex_generate_single_device(model, tokenizer, sampler, task, prompt, device)
...
def ipex_generate_simple(
    ...
    try:
        ...
        try:
            ...
            pass
        raise IPEXMemoryError(
            f"Intel GPU out of memory during simple generation: {e}",
            device_id=0
        )
    except Exception as e:
        ...
        try:
            ...
            pass
        raise IPEXInferenceError(
            f"IPEX simple generation failed: {e}",
            device_id=0,
            step="simple_generation"
        )