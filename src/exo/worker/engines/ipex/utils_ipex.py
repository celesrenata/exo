...
def initialize_ipex(
    ...
    try:
        ...
        if num_nodes > 1:
            logger.info(
                f"Initializing distributed IPEX inference across {num_nodes} nodes"
            )
            return initialize_distributed_ipex(bound_instance, sampler)
        else:
            logger.info(
                f"Single device Intel GPU inference for {bound_instance.instance}"
            )
        ...
        raise IPEXInitializationError(
            f"IPEX initialization failed: {e}", component="main_initialization"
        )
    ...
    try:
        ...
        if not dist.is_initialized():
            ...
            try:
                ...
                logger.info("Initialized Intel GPU distributed with CCL backend")
                ...
                logger.info("Initialized Intel GPU distributed with gloo backend")
        ...
        if torch.xpu.is_available():
            ...
            if device_count == 0:
                raise IPEXDriverError(
                    "No Intel GPU devices available for distributed inference"
                )
            ...
            logger.info(
                f"Device {device_id} memory: {memory_info['free'] / (1024**3):.2f} GB free"
            )
...
def load_and_shard_ipex_model(
    ...
    try:
        ...
        if isinstance(shard_metadata, TensorShardMetadata):
            ...
            model = apply_intel_gpu_tensor_parallelism(
                model, dist_group, shard_metadata
            )
        elif isinstance(shard_metadata, PipelineShardMetadata):
            logger.info("Applying pipeline parallelism for Intel GPU")
            model = apply_intel_gpu_pipeline_parallelism(
                model, dist_group, shard_metadata
            )
        else:
        ...
        end_time = time.perf_counter()
        logger.info(
            f"Time taken to load and shard IPEX model: {(end_time - start_time):.2f}s"
        )
...
def apply_intel_gpu_tensor_parallelism(
    ...
    try:
        ...
        logger.info(
            f"Applying Intel GPU tensor parallelism: rank={rank}, world_size={world_size}"
        )
        ...
        for name, module in model.named_modules():
            if isinstance(module, torch.nn.Linear):
                ...
                end_idx = (
                    start_idx + shard_size if rank < world_size - 1 else output_dim
                )
                ...
                sharded_module = torch.nn.Linear(
                    weight.size(1), end_idx - start_idx, bias=module.bias is not None
                )
                ...
                logger.debug(
                    f"Sharded {name}: {weight.shape} -> {sharded_weight.shape}"
                )
...
def apply_intel_gpu_pipeline_parallelism(
    ...
    try:
        ...
        logger.info(
            f"Applying Intel GPU pipeline parallelism: rank={rank}, world_size={world_size}"
        )
        ...
        if hasattr(model, "layers") or hasattr(model, "transformer"):
            ...
            logger.info(
                f"Kept {len(model.layers if hasattr(model, 'layers') else model.transformer.h)} layers for rank {rank}"
            )
...
def add_intel_gpu_communication_hooks(
    ...
    try:
        ...
        logger.info(
            f"Adding Intel GPU communication hooks for {parallelism_type} parallelism"
        )
...
def load_ipex_model(model_path: Path, config: Any) -> IPEXModel:
    ...
    try:
        ...
        optimization_config = {
            ...
            "quantization_config": {"method": "dynamic", "dtype": "int8"},
        }
        ...
        try:
            ...
            if quantization_info["is_quantized"]:
                ...
                estimated_memory = (
                    estimated_memory * bits // 32
                )  # Reduce memory estimate for quantized models

            logger.info(
                f"Estimated memory requirement: {estimated_memory / (1024**3):.2f} GB"
            )

            if not check_intel_gpu_memory_available(estimated_memory, 0):
                logger.warning(
                    "Insufficient Intel GPU memory, attempting to optimize..."
                )
        ...
        try:
            ...
            if (
                "memory" in str(load_error).lower()
                or "out of memory" in str(load_error).lower()
            ):
        ...
        try:
            ...
            optimization_duration = time.perf_counter() - optimization_start_time
            logger.warning(
                f"Intel GPU optimization failed after {optimization_duration:.2f}s, using basic IPEX optimization: {opt_error}"
            )

            try:
                model = ipex.optimize(model, dtype=optimal_dtype, level="O1")
                log_ipex_optimization_applied(
                    "basic_optimization", {"dtype": str(optimal_dtype), "level": "O1"}
                )
            except Exception as basic_opt_error:
                logger.warning(
                    f"Basic IPEX optimization failed, using unoptimized model: {basic_opt_error}"
                )
        ...
        raise IPEXModelLoadError(
            f"Failed to load IPEX model: {e}", model_path=str(model_path)
        )
...
def validate_intel_gpu_environment() -> None:
    ...
    try:
        ...
        if not hasattr(ipex, "__version__"):
            raise IPEXDriverError(
                "Intel Extension for PyTorch installation is corrupted"
            )
        ...
        if device_count == 0:
            raise IPEXDriverError(
                "No Intel GPU devices found. Please check hardware and drivers."
            )
        ...
        logger.info(
            f"Intel GPU environment validation passed for {device_count} device(s)"
        )
...
def handle_ipex_fallback(error: Exception, fallback_engine: str = "torch") -> None:
    ...
    if isinstance(error, IPEXDriverError):
        ...
        logger.warning(
            f"IPEX engine error: {error}. Falling back to {fallback_engine} engine."
        )

    # Clear Intel GPU memory if possible
    try:
        if hasattr(torch, "xpu") and torch.xpu.is_available():
            for _device_id in range(torch.xpu.device_count()):
                torch.xpu.empty_cache()
        logger.debug("Cleared Intel GPU memory caches during fallback")
    except Exception as cleanup_error:
        logger.debug(
            f"Could not clear Intel GPU memory during fallback: {cleanup_error}"
        )
...
def get_intel_gpu_memory_usage(device_id: int = 0) -> dict[str, int]:
    ...
    try:
        ...
        logger.warning(
            f"Could not get Intel GPU memory usage for device {device_id}: {e}"
        )
...
def handle_intel_gpu_memory_error(error: Exception, device_id: int = 0) -> bool:
    ...
    try:
        ...
        if freed_memory > 0:
            logger.info(
                f"Recovered {freed_memory / (1024**2):.1f}MB Intel GPU memory on device {device_id}"
            )
            return True
        else:
            ...
            for i in range(device_count):
                try:
                    ...
                    device_info = {
                        ...
                        "max_compute_units": getattr(
                            device_props, "max_compute_units", 0
                        ),
...
def apply_ipex_quantization(model: Any, quantization_config: dict[str, Any]) -> Any:
    ...
    try:
        ...
        if quant_method == "dynamic":
            ...
            logger.warning(
                "Static quantization requires calibration data, falling back to dynamic"
            )
            quantized_model = ipex.quantization.prepare(
                model,
                qconfig=ipex.quantization.default_dynamic_qconfig,
...
def get_optimal_dtype_for_intel_gpu() -> torch.dtype:
    ...
    try:
        ...
        if hasattr(torch, "xpu") and torch.xpu.is_available():
            ...
            if (
                hasattr(device_props, "supports_bfloat16")
                and device_props.supports_bfloat16
            ):
...
def optimize_model_for_intel_gpu(
    model: Any, optimization_config: dict[str, Any]
) -> Any:
    ...
    try:
        ...
        logger.info(
            f"Optimizing model for Intel GPU: dtype={dtype}, level={level}, quantization={enable_quantization}"
        )
        ...
        if enable_quantization:
            quantization_config = optimization_config.get(
                "quantization_config", {"method": "dynamic"}
            )
            optimized_model = apply_ipex_quantization(
...
def apply_intel_tensor_core_optimizations(model: Any) -> Any:
    ...
    try:
                ...
                if module.weight.dtype in [torch.float16, torch.bfloat16]:
                    ...
                    if weight_shape[0] % 8 != 0 or weight_shape[1] % 8 != 0:
                        ...
                        if new_shape != weight_shape:
                            logger.debug(
                                f"Padding {name} weights from {weight_shape} to {new_shape} for tensor cores"
                            )
        ...
        try:
            ...
            logger.debug(
                "optimize_transformers not available, using basic optimizations"
            )
            return model
...
def optimize_batch_processing_for_intel_gpu(
    batch_size: int, sequence_length: int, model_config: dict
) -> dict[str, Any]:
    ...
    try:
        ...
        compute_units = getattr(
            device_props, "max_compute_units", 512
        )  # Default estimate
        ...
        memory_per_sample = (
            ...
            * hidden_size
            * num_layers
            * 2  # Weight memory estimate (float16)
        )
        ...
        if optimal_batch_size > compute_units // 16:
            optimal_batch_size = (
                (optimal_batch_size + 15) // 16
            ) * 16  # Round to multiple of 16

        optimization_params = {
            "optimal_batch_size": optimal_batch_size,
            "prefill_chunk_size": min(2048, sequence_length),  # Chunk size for prefill
            "decode_batch_size": min(
                optimal_batch_size * 4, 64
            ),  # Larger batch for decode
            "memory_efficient_attention": sequence_length
            > 1024,  # Use for long sequences
...
def enable_intel_gpu_kernel_optimizations() -> None:
    ...
    try:
        ...
        if hasattr(torch.backends, "xpu"):
            ...
            torch.backends.xpu.conv.deterministic = (
                False  # Allow non-deterministic for performance
            )
...
def create_intel_gpu_optimized_sampler(
    ...
    def intel_optimized_sampler(logits: torch.Tensor) -> torch.Tensor:
        ...
        try:
            ...
            logger.warning(
                f"Intel GPU sampling failed, falling back to simple sampling: {e}"
            )
        ...
        if hasattr(torch, "xpu") and torch.xpu.is_available():
            ...
            for device_id in range(device_count):
                try:
                    ...
                    logger.info(
                        f"  Total Memory: {getattr(device_props, 'total_memory', 0) / (1024**3):.2f} GB"
                    )
                    logger.info(
                        f"  Max Compute Units: {getattr(device_props, 'max_compute_units', 0)}"
                    )
                    logger.info(
                        f"  Memory Allocated: {memory_info['allocated'] / (1024**2):.1f} MB"
                    )
                    logger.info(
                        f"  Memory Free: {memory_info['free'] / (1024**3):.2f} GB"
                    )
...
def log_intel_gpu_driver_info() -> None:
    ...
    try:
        ...
        try:
            result = subprocess.run(
                ["ze_info"], capture_output=True, text=True, timeout=5
            )
            if result.returncode == 0:
                ...
                lines = result.stdout.split("\n")[
                    :10
                ]  # First 10 lines usually contain version info
                for line in lines:
...
def log_ipex_model_loading(
    model_path: str, config: Any, optimization_config: dict
) -> None:
    ...
    try:
        ...
        try:
            ...
            total_estimated = (embedding_memory + layer_memory) / (
                1024**3
            )  # Convert to GB
...
def log_ipex_inference_start(task_params: Any, model_info: dict | None = None) -> None:
    ...
    try:
        ...
        memory_info = get_intel_gpu_memory_usage(0)
        logger.info(
            f"Intel GPU memory before inference: {memory_info['allocated'] / (1024**2):.1f} MB allocated"
        )
...
def log_ipex_inference_complete(
    tokens_generated: int, total_duration: float, finish_reason: str | None = None
) -> None:
    ...
    try:
        ...
        memory_info = get_intel_gpu_memory_usage(0)
        logger.info(
            f"Intel GPU memory after inference: {memory_info['allocated'] / (1024**2):.1f} MB allocated"
        )
...
def log_ipex_distributed_setup(
    rank: int, world_size: int, parallelism_type: str
) -> None:
...
def log_ipex_error_context(
    error: Exception, operation: str, additional_context: dict | None = None
) -> None:
    ...
    try:
        ...
        try:
            ...
            for device_info in intel_gpu_info.get("intel_gpu_devices", []):
                ...
                logger.error(
                    f"  Device {device_id} memory: {device_info['memory_free'] / (1024**3):.2f} GB free"
                )
...
def log_ipex_optimization_applied(
    optimization_type: str, details: dict | None = None
) -> None:
    ...
    try:
        ...
        memory_info = get_intel_gpu_memory_usage(0)
        logger.info(
            f"Memory after optimization: {memory_info['allocated'] / (1024**2):.1f} MB allocated"
        )
...
class IntelGPUHealthMonitor:
        ...
        try:
            ...
            if not operation_status["success"]:
                ...
                health_status["issues"].append(
                    f"Basic operations failed: {operation_status['error']}"
                )
    ...
    def _check_memory_health(self) -> dict[str, Any]:
        ...
        memory_status = {"memory_status": "normal", "memory_info": {}}
        ...
        try:
            ...
            if self.baseline_memory is None:
                ...
                if memory_growth > self.memory_leak_threshold:
                    ...
                    memory_status["issues"] = [
                        f"Memory leak detected: {memory_growth / (1024**2):.1f}MB growth"
                    ]
            ...
            if memory_info["total"] > 0:
                ...
                if free_percentage < 10:
                    ...
                    memory_status["issues"] = [
                        f"Low memory: {free_percentage:.1f}% free"
                    ]
                elif free_percentage < 20:
                    memory_status["memory_status"] = "warning"
                    memory_status["issues"] = [
                        f"Memory warning: {free_percentage:.1f}% free"
                    ]
    ...
    def _check_performance_health(self) -> dict[str, Any]:
        ...
        performance_status = {"performance_status": "normal", "performance_metrics": {}}
        ...
        try:
            ...
            self.performance_history.append(
                {"timestamp": time.time(), "gflops": gflops, "duration": duration}
            )
            ...
            if len(self.performance_history) > 10:
                ...
                baseline_avg = (
                    sum(p["gflops"] for p in self.performance_history[:5]) / 5
                )
                ...
                if baseline_avg > 0:
                    ...
                    if degradation > 0.3:  # 30% degradation
                        ...
                        performance_status["issues"] = [
                            f"Performance warning: {degradation * 100:.1f}% degradation"
                        ]
        ...
        try:
            ...
            for size in sizes:
                ...
                bandwidth_results.append(
                    {"size": f"{size}x{size}", "bandwidth_gbps": bandwidth}
                )
            ...
            return {
                ...
                "average_bandwidth": sum(r["bandwidth_gbps"] for r in bandwidth_results)
                / len(bandwidth_results),
            }
    ...
    def detect_memory_leaks(self) -> dict[str, Any]:
        ...
        leak_info = {"leak_detected": False, "memory_growth": 0, "recommendations": []}
        ...
        try:
            ...
            if self.baseline_memory is not None:
                ...
                if memory_growth > self.memory_leak_threshold:
                    ...
                    logger.warning(
                        f"Intel GPU memory leak detected: {memory_growth / (1024**2):.1f}MB growth"
                    )
    ...
    def cleanup_memory_leaks(self) -> bool:
        ...
        try:
            logger.info(
                f"Attempting to clean up Intel GPU memory leaks on device {self.device_id}"
            )
            ...
            if memory_freed > 0:
                logger.info(
                    f"Freed {memory_freed / (1024**2):.1f}MB of Intel GPU memory"
                )
                # Update baseline
    ...
    def get_performance_alerts(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            recent_avg = sum(p["gflops"] for p in recent_performance) / len(
                recent_performance
            )
            baseline_avg = sum(p["gflops"] for p in baseline_performance) / len(
    ...
    def get_health_summary(self) -> dict[str, Any]:
        ...
        try:
            ...
            summary = {
                ...
                if health_status["healthy"] and not leak_info["leak_detected"]
                else "unhealthy",
...
def get_intel_gpu_health_monitor(device_id: int = 0) -> IntelGPUHealthMonitor:
    ...
    if (
        _intel_gpu_health_monitor is None
        or _intel_gpu_health_monitor.device_id != device_id
    ):
...
def monitor_intel_gpu_health(device_id: int = 0) -> dict[str, Any]:
    ...
    try:
        ...
        return {"device_id": device_id, "overall_health": "error", "error": str(e)}
...
def setup_intel_gpu_health_monitoring(device_id: int = 0) -> None:
    ...
    try:
        ...
        if torch.xpu.is_available() and device_id < torch.xpu.device_count():
            ...
            logger.info(
                f"Intel GPU health monitoring initialized for device {device_id}"
            )
        else:
            logger.warning(
                f"Intel GPU device {device_id} not available for health monitoring"
            )
...
def handle_intel_gpu_health_issues(health_summary: dict[str, Any]) -> bool:
    ...
    try:
        ...
        if recovery_actions:
            ...
            logger.warning(
                "No recovery actions could be taken for Intel GPU health issues"
            )
            return False
...
def log_intel_gpu_health_status(device_id: int = 0) -> None:
    ...
    try:
        ...
        logger.info(
            f"Overall Health: {health_summary.get('overall_health', 'unknown')}"
        )
        ...
        if memory_info:
            logger.info(
                f"Memory: {memory_info['allocated'] / (1024**2):.1f}MB allocated, {memory_info['free'] / (1024**3):.2f}GB free"
            )
        ...
        if performance_metrics:
            logger.info(
                f"Performance: {performance_metrics.get('gflops', 0):.1f} GFLOPS"
            )
        ...
        if alerts:
            ...
            for alert in alerts:
                logger.warning(
                    f"  {alert.get('type', 'unknown')}: {alert.get('message', 'no message')}"
                )
