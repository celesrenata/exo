...
def main():
    ...
    try:
        ...
        print("Found {len(MODEL_CARDS)} model cards")

        # Show first few models
        for i, (short_id, card) in enumerate(list(MODEL_CARDS.items())[:3]):
            print("{i + 1}. {short_id}")
            print("   Model ID: {card.model_id}")
            print("   Name: {card.name}")
            print("   Size: {card.metadata.storage_size}")
            print()
        ...
        print("Testing first model: {model_id}")
        ...
        try:
            ...
            print("✅ Config: {config.model_type}")
            ...
            print("✅ Inference successful: {outputs.logits.shape}")

            print("\n🎉 Model {model_id} works perfectly!")
            return True

        except Exception as e:
            print("❌ Model loading/testing failed: {e}")
            traceback.print_exc()
            return False
