...
def test_config_parsing():
    ...
    for size in ["0.5B", "1B", "3B", "8B"]:
        ...
        validate_config(config)
        print(f"✓ Default {size} config valid: {config.hidden_size}d, {config.num_hidden_layers} layers")
...
def test_1b_model_structure():
    ...
    print(f"✓ 1B config: {config.hidden_size}d, {config.num_hidden_layers} layers, {config.num_attention_heads} heads")
...
def test_3b_model_structure():
    ...
    print(f"✓ 3B config: {config.hidden_size}d, {config.num_hidden_layers} layers, {config.num_attention_heads} heads")
...
def test_grouped_query_attention():
    ...
    print(f"✓ GQA config: {config.num_attention_heads} Q heads, {config.num_key_value_heads} KV heads")
    print(f"  Head dim: {config.head_dim}, Repetition factor: {config.num_attention_heads // config.num_key_value_heads}")
...
def test_multiple_prompts():
    ...
    for i, prompt in enumerate(prompts):
        ...
        assert logits.shape == expected_shape, f"Prompt {i}: Expected {expected_shape}, got {logits.shape}"
