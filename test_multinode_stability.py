...
def test_multinode_inference():
        ...
        if state_response.status_code == 200:
            ...
            for instance_id, instance_data in instances.items():
                if 'CpuRingInstance' in instance_data:
                    ...
                    print(f"  Instance {instance_id[:8]}...: world_size={world_size}")
        else:
            print(f"❌ State check failed: {state_response.status_code}")
            
    except Exception as e:
        print(f"❌ State check exception: {e}")
    ...
    if success_count == total_requests:
        ...
        print(f"⚠️  PARTIAL SUCCESS: {success_count}/{total_requests} requests succeeded")
